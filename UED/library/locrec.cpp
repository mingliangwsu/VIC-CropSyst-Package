...
, Geolocation()
{}
//_Geolocation_record:constructor______________________________________________/
Geolocation_record::Geolocation_record(const Geolocation& copy_from)
: Record_base(UED_geolocation_information)
, Geolocation(copy_from)
{}
//_Geolocation_record:constructor___________ _______________________2005-05-03_/
...
nat32 Geolocation_record::compute_record_body_size()           const
{ // Returns the size of the record body (not including the header).
   nat32 size = sizeof(available_data);
   if (has_geocoordinates())     size += sizeof(latitude_dec_deg)
                                        +sizeof(longitude_dec_deg)
                                        +sizeof(elevation);
   if (has_screening_height())   size += sizeof(screening_height_32);            //100519
   if (has_geopolitical_codes()) size += sizeof(country_code_ISO3166)
                                        +sizeof(state_code)
                                        +sizeof(county_code);
   if (has_geopolitical_names()) size += (country_name.length()+1)
                                        +(state_name.length()+1)
                                        +(county_name.length()+1);
   if (has_station_number())     size += sizeof(Geolocation::ID);
...
//_compute_record_body_size_________________________________________2000-10-08_/
bool Geolocation_record::read_body(Binary_file_interface &parent_database)
{  available_data = parent_database.read_nat32();
   ...
   if (has_station_number())                                                     //031217
      ID = parent_database.read_nat32();                                         //031217
...
void Geolocation_record::write_body(Binary_file_interface &parent_database)
{  parent_database.write_nat32(available_data);
   ...
   if (has_station_number())                                                     //031217
      parent_database.write_nat32(ID);                                           //031217
   if (has_station_ID_code())                                                    //031217
...
//_is_more_complete_than____________________________________________2014-06-08_/
using namespace std;
#ifdef UED_DUMP
