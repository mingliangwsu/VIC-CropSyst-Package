...
double calc_atmos_energy_bal(//const energy_bal_struct &snow_energy,
                             double  InOverSensible,
...
************************************************************************/
  ...
  F = 1;                                                                  // canopy closure fraction, not currently used by VIC
...
double solve_atmos_energy_bal(double Tcanopy, ...)
{
  va_list ap;
  va_start(ap, Tcanopy);
  double error = func_atmos_energy_bal(Tcanopy, ap);
  va_end(ap);
  return error;
}
//______________________________________________________________________________
double error_calc_atmos_energy_bal(double Tcanopy, ...)
{
  va_list ap;
  va_start(ap, Tcanopy);
  double error = error_print_atmos_energy_bal(Tcanopy, ap);
  va_end(ap);
  return error;
}
//______________________________________________________________________________
double error_print_atmos_energy_bal(double Tcanopy, va_list ap)
{
  ...
  double LatentHeat    = (double)  va_arg(ap, double);
  double NetRadiation  = (double)  va_arg(ap, double);
  double Ra            = (double)  va_arg(ap, double);
  double Tair          = (double)  va_arg(ap, double);
  double atmos_density = (double)  va_arg(ap, double);
  double InSensible    = (double)  va_arg(ap, double);
  double *SensibleHeat = (double *)va_arg(ap, double *);
  char *ErrorString    = (char *)va_arg(ap, char *);
...
double error_calc_atmos_moist_bal(double VPcanopy, ...) {
  va_list ap;
  va_start(ap, VPcanopy);
  double error = error_print_atmos_moist_bal(VPcanopy, ap);
  va_end(ap);
  return error;
}
//______________________________________________________________________________
...
{
  // extract variables from va_arg
  double  InLatent      = (double)  va_arg(ap, double);
  double  Lv            = (double)  va_arg(ap, double);
  double  Ra            = (double)  va_arg(ap, double);
  double  atmos_density = (double)  va_arg(ap, double);
  double  gamma         = (double)  va_arg(ap, double);
  double  vp            = (double)  va_arg(ap, double);
  double *AtmosLatent   = (double *)va_arg(ap, double *);
  char *ErrorString     = (char *)  va_arg(ap, char *);
