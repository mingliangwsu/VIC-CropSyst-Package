...
//190130RLN doesnt appear to be needed
#include <strings.h>
#endif
#include <vicNl.h>
#include <stdarg.h>
...
//190130RLN logf takes single precision but it is being used with a double
#define logf(a) log((a))
#endif
...
******************************************************************/
  ...
  if (options.FROZEN_SOIL && soil_con->FS_ACTIVE)
    find_0_degree_fronts(soil_con->Zsum_node, T, Nnodes,energy);
  else
    energy->Nfrost = 0;
  ...
  if (options.QUICK_FLUX) {
    ErrorFlag = estimate_layer_ice_content_quick_flux(soil_con->depth, soil_con->dp,
  ...
  **********************************************************************/
  ...
  while(!Done && Error==0 && ItCount<MAXIT) {
...
#else
    T[j] = root_brent(T0[j]-(param.SOIL_DT), T0[j]+(param.SOIL_DT),
              ErrorString, soil_thermal_eqn,
              T[j+1], T[j-1], T0[j], moist[j], max_moist[j],
              bubble[j], expt[j],
#if EXCESS_ICE
    ...
    T[Nnodes-1] = root_brent(T0[Nnodes-1]-SOIL_DT, T0[Nnodes-1]+SOIL_DT,
                 ...
                 A[j], B[j], C[j], D[j], E[j], EXP_TRANS, j);
#else
    T[Nnodes-1] = root_brent(T0[Nnodes-1]-param.SOIL_DT, T0[Nnodes-1]+param.SOIL_DT,
                 ErrorString, soil_thermal_eqn, T[Nnodes-1],
                 T[Nnodes-2], T0[Nnodes-1],
                 moist[Nnodes-1], max_moist[Nnodes-1],
