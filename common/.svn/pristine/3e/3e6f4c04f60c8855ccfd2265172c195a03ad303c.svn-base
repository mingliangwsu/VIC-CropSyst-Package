...
//_Weather_column_data______________________________________________1999-10-13_/
/*
Bit
0     DOY
1     precipitation
...
//140716    float64 get_solar_radiation              (CORN::DOY i_DOY, CORN::Quality &quality) const ; //  MJ/m2/day          //991013
 ...
 public: 	   // The following sets do not let inferior quality values to be assigned
   ...
   float64 set_wind_speed_m_d (CORN::DOY i_DOY,float64 i_windspeed    , CORN::Quality &quality); //  m/day (stored as m/S)
   float64 set_solar_rad    (CORN::DOY i_DOY,float64 i_solar_rad      , CORN::Quality &quality); //  MJ/m2/day
   float64 set_cloud_cover_percent(CORN::DOY i_DOY,float64 i_cloud_cover_percent , CORN::Quality &quality); //  %          //991022
   float64 set_pot_ET_m     (CORN::DOY i_DOY,float64 i_pot_ET         , CORN::Quality &quality); //  m   (stored as mm)
 ...
 private:
   DOY read_ASCII_record                                                         //040427
       ...
       ,CORN::Date_interface &expected_line_date    // Not const because we relax if the full date was read (V4 text files)
       ,CORN::Date_interface &actual_line_date);
   bool read_ASCII();
   ...
   Weather_column_data *get_data_set
      (UED::Variable_code var_code,Units_code units_code = 0
      , bool desired_for_output =false)                               mutation_; //020905
               // Allocate if necessary
};
//_Weather_text_file___________________________________________________________/
#endif
// wthrfile.h
