...
//_Mass_common_________________________________________________________________/
class Water
...
{public:
 ...
 public:
   ...
   inline virtual float64 gallon()                                        const
                          { return m3() * 264.172052358148; } // m^3 -> gallon
   inline virtual bool clear()                                    modification_
                                        {return Physical::Mass_common::clear();}
   inline virtual bool is_empty()                                         const
                                   { return Physical::Mass_common::is_empty(); }
   inline virtual const Physical::Mass &ref_mass()        const { return *this;}
   inline virtual float64 get_temperature_Celcius()                       const
                                                { return temperature.Celcius();} //150529
   inline virtual float64 set_temperature_Celcius(float64 temp_C)  modification_
             { return temperature.set_Celcius(temp_C, CORN::estimated_quality);} //150529
   inline virtual const Water &get_water()              const { return (*this);}
};
//_Water____________________________________________________________2011-06-30_/
...
{  // This is a mass that is part of something else
 ...
 public:
   Constituent_of_mass(float64 _amount, CORN::Units_code _units
      ,const Physical::Mass &of_mass);
   Constituent_of_mass(float64 _amount, CORN::Units_code _units
      ,const Physical::Mass *of_mass);
   ...
   inline virtual float64 set_concentration_percent
      (float64 concentration_percent)                              modification_
      {  return set_concentration_fraction(concentration_percent/100.0); }
   inline virtual float64 set_concentration_fraction
      (float64 concentration_fraction)                             modification_
   ...
   inline virtual const Physical::Mass *know_whole_mass
      (const Physical::Mass *_whole_mass)                          modification_ //110803
...
//_Constituent_of_mass_________________________________________________________/
...
{  // This is a mass that is part of something else
 ...
 public:
   Constituent_of_matter(float64 _amount, CORN::Units_code _units
      ,const Matter &of_matter);
   Constituent_of_matter(float64 _amount, CORN::Units_code _units
      ,const Matter *of_matter);
   ...
   */
    inline const Matter *know_whole_matter(const Matter *whole_)   modification_ //110803
       { know_whole_mass(&whole_->ref_mass());
         return whole_matter = whole_;
       }
 protected:  // This constructor is just to make the compiler happy
...
//_Constituent_of_matter_______________________________________________________/
...
public:
   Mass_elemental
      (float64 _amount, CORN::Units_code _units
      ,const Matter &of_matter ,float64 _atomic_mass);
   Mass_elemental
      (float64 _amount, CORN::Units_code _units
      ,const Matter *of_matter,float64 _atomic_mass);
   Mass_elemental
...
//_Mass_elemental______________________________________________________________/
