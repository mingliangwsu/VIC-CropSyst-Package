...
,  emergence_date()
,  flowering_date()
,  tuberization_date()                                                           //181111
,  yield_formation_date()                                                        //181111
,  culminescence_date()                                                          //181111
,  maturity_date()
,  harvest_date()
#ifdef CROP_ORCHARD
,  bud_break_date()                                                              //081013
,  fructescence_date()                                                           //181111
...
,  avg_filling_duration_index(0)
#endif
, avg_tuber_init_temp_stress_index(0)
, avg_tuber_filling_temp_stress(0)
#ifdef NITROGEN
,  N_leached(0)
,  N_total_applied(0)
,  N_auto_uptake(0)
...
,  Tavg_mean(0)
,  days_to_flowering(0)
,  duration_season_days(0)
,  GDDs_normal(0)
,  canopy_cover_max(0)
,  canopy_cover_green_final(0)
,  canopy_cover_total_final(0)
{}
//_Season_data_record:constructor______________________________________________/
...
//_clear____________________________________________________________2011-01-15_/
void Season_data_record::update
(const CropSyst::Crop_model_interface &crop                                      //131008_130717
,const CropSyst::Soil_components &soil
,CropSyst::Period_accumulators    *GP_accumulators
#ifdef NITROGEN
,Chemical_balance_accumulators_molecular  *NO3_GP_bal_accums_M                   //050114
,Chemical_balance_accumulators_molecular  *NH4_GP_bal_accums_M                   //050114
   // accums are 0 if not simulating N
#endif
...
//   const CropSyst::Phenology &phenology = crop.ref_phenology();                  //181111
   ...
   restart_date                     .set_date32(phenology.get_restart_date());   //041205
   emergence_date                   .set_date32(phenology.get_emergence_date());
   flowering_date                   .set_date32(phenology.get_anthesis_date());  //181111
   tuberization_date .set_date32(phenology.get_tuberization_date());             //081111
   yield_formation_date
      .set_date32(
         /*190617
         #ifdef CROP_ORCHARD
          (crop.get_crop_model() == CROPSYST_ORCHARD_MODEL)
          ? phenology.get_fructescence_date() :
         #endif
         */
          phenology     .get_yield_formation_date());
   culminescence_date   .set_date32(phenology.get_culminescence_date());         //181111_180413
   maturity_date        .set_date32(phenology.get_maturity_date());
   inactive_begin_date  .set_date32(phenology.get_quiescence_init_date());       //081013
   inactive_end_date    .set_date32(phenology.get_quiescence_culm_date());       //081013

   const CropSyst::Crop_orchard_fruit *fruit_orchard_crop                        //190620
   = dynamic_cast<const CropSyst::Crop_orchard_fruit *>(&crop);                  //190620
   if (fruit_orchard_crop)                                                       //190620
   {
   //190619 #ifdef CROP_ORCHARD
   bud_break_date         .set_date32(fruit_orchard_crop->get_bud_break_date()); //200325
   fructescence_date             .set_date32(phenology.get_fructescence_date()); //181111
   rapid_fruit_growth_date .set_date32(phenology.get_rapid_fruit_growth_date()); //081013
   //#if (!PHENOLOGY_VERSION || (PHENOLOGY_VERSION==2013))
   chill_requirement_satisfied_date .set_date32(fruit_orchard_crop->get_chill_requirement_satisfied_date());  //081013
   //#endif
   // PHENOLOGY_2018 need to implement chill_requirement_satisfied_date
   //190619 #endif
   {
      yield= fruit_canopy_biomass = per_ha_to_per_m2(fruit_orchard_crop->get_fruit_dry_biomass_kg_ha()); //200608
      nonfruit_canopy_biomass    = per_ha_to_per_m2(fruit_orchard_crop->get_canopy_biomass_kg_ha());  //200608
      fruit_reserves_biomass     = fruit_orchard_crop->get_fruit_reserves_biomass_kg_m2();              //200608
   }
   }
...
*/
   ...
   {  // if in dormancy there might not be biomass_fate_season;                  //051228
      ...
      residue_biomass   = biomass_fate_season->chaff_or_leaf_litter
                         +biomass_fate_season->stubble_dead;                     //040920
      disposed_biomass  = biomass_fate_season->disposal;                         //040920
   }
   dry_biomass = crop.ref_reported().season.biomass_production;                  //190705
      //190705 crop.get_total_season_biomass_production_kg_m2();
   ...
   {
      ET_max                     =GP_accumulators->evapotranspiration_max;
      //190812 ET_pot                     =GP_accumulators->pot_evapotranspiration;
      ET_act                     =GP_accumulators->evapotranspiration_act;
      ET_ref                     =GP_accumulators->evapotranspiration_ref;       //130723
      transp_pot                 =GP_accumulators->pot_transpiration;
      transp_act                 =CORN::must_be_0_or_greater<float32>
                                    (GP_accumulators->act_transpiration);
   ...
   if (NO3_GP_bal_accums_M && NH4_GP_bal_accums_M)                               //050114
   {
      N_leached = NO3_GP_bal_accums_M->get_reported_leached(true)                //981230
                 +NH4_GP_bal_accums_M->get_reported_leached(true);               //981230
      N_inorganic_applied                                                        //160323
         = NO3_GP_bal_accums_M->get_applied(true)
          +NH4_GP_bal_accums_M->get_applied(true);
      ...
      N_available                  =
         (NO3_GP_bal_accums_M->get_original_profile_content(true)
         +NH4_GP_bal_accums_M->get_original_profile_content(true)
         +NH4_GP_bal_accums_M->get_transformed_from(true)//{ GP_mineralization}
         +NO3_GP_bal_accums_M->get_applied(true)
         +NH4_GP_bal_accums_M->get_applied(true)
      ...
      {  N_mineralized_OM           = GP_accumulators->OM_N_mineralization;      //970814
         ...
         //200709 N_mass_yield               = GP_accumulators->N_mass_yield;             //120117
   ...
   if (crop_N_kg_m2)                                                             //050722
   {
      ...
      if (!CORN::is_zero<float32>(N_max_expected_conc_at_maturity)) // Could rarely be the case if the season ends before maturity//061214
...
//NYI   P_applied   = xxxxx
...
#endif
      N_mass_yield = crop.ref_reported().season.N_mass_yield;                    //100709
   ...
   if (NO3_GP_bal_accums_M && NH4_GP_bal_accums_M)                               //041005
   {  denitrification = NO3_GP_bal_accums_M->get_transformed_to(true);
      nitrification = NH4_GP_bal_accums_M->get_transformed_to(true);             //160323
      ...
      N_soil_profile_final                                                       //161130
         = NO3_GP_bal_accums_M->get_current_profile_content(true)
         + NH4_GP_bal_accums_M->get_current_profile_content(true);      // Mineral N in the soil    (mineral profile N)
   }
#ifdef NYI
   ...
   degree_hours_above_30_season                                                  //180919
      = crop_complete.above_30C_season ? (float32)crop_complete.above_30C_season->degree_hours_of_extreme.sum() : 0;
   degree_hours_above_35_season                                                  //180919
      = crop_complete.above_35C_season ? (float32)crop_complete.above_35C_season->degree_hours_of_extreme.sum() : 0;
   #endif
   // NYI I have new method for flowering_fortnight
   ...
   */
   #endif
   duration_season_days       = crop.ref_phenology().get_season_duration_days(); //181111
   canopy_cover_max           = crop_complete.reported.season.canopy_cover_max;  //190705 reported_canopy_cover_max;         //180413
   canopy_cover_green_final   = crop_complete.reported.daily.canopy_cover_green; //190705 reported_canopy_cover_green;       //180413
   canopy_cover_total_final   = crop_complete.reported.daily.canopy_cover_total; //190705 reported_canopy_cover_green;       //180413

   GDDs_normal                = crop_complete.get_accum_degree_days();           //200722
}
...
{  bool expected = CORN::Data_record::expect_structure(for_write);
   ...
   expect_int32   ("tuber_init_date"   ,tuberization_date   .mod_date32());      //081111
   expect_int32   ("filling_date"      ,yield_formation_date.mod_date32());
   expect_int32   ("peak_LAI_date"     ,culminescence_date  .mod_date32());
   ...
   expect_int32("initial_fruit_growth_date",fructescence_date        .mod_date32());
   ...
   expect_float32 ("reserves_biomass"              ,fruit_reserves_biomass);     //200618
#endif
   ...
   expect_float32 ("ET_max"                        ,ET_max);
   //190812expect_float32 ("ET_pot"                        ,ET_pot);
   ...
   */
...
#endif
   #ifdef CROPSYST_PROPER
   expect_float32 ("DegHr_above_30",degree_hours_above_30_flowering_fortnight);  //flowering
   expect_float32 ("DegHr_above_34",degree_hours_above_34_flowering_fortnight);  //flowering
   expect_float32 ("DegHr_above_30_season",degree_hours_above_30_season);        //180919
   expect_float32 ("DegHr_above_35_season",degree_hours_above_35_season);        //180919
   #endif
   // NYI reimplement the 30 and 34 degree hours for flowering
   ...
   expect_float32 ("duration_season"   ,duration_season_days);                   //160523
   ...
   expect_float32 ("canopy_cover_total_final",canopy_cover_total_final);         //200127
   expect_float32 ("thermal_time"            ,GDDs_normal);                      //200722
...
//    The following are dates
   ...
   add_field2(season_section,"reserves_biomass"             ,CSVC_crop_base_fruit_reserves_biomass,UC_kg_m2,UT_arbitrary_date);              //200518
#endif
...
// The following are accumulated values
   ...
   add_field2(season_section,"ET_max"                        ,CSVC_weather_ET_max_period_sum                ,UC_m    ,UT_arbitrary_date);
   //190812 add_field2(season_section,"ET_pot"                        ,CSVC_weather_ET_pot_period_sum                ,UC_m    ,UT_arbitrary_date);
...
//161130   add_field2(season_section,"N_soil_profile"                ,CSVC_soil_N_N_amount                          ,UC_kg_m2,UT_arbitrary_date);
   ...
   add_field2(season_section,"DegHr_above_30_season",0x8D28059E,UC_Celcius_degree_hours,UT_arbitrary_date); //180919
   add_field2(season_section,"DegHr_above_35_season",0x8D2805A3,UC_Celcius_degree_hours,UT_arbitrary_date); //180919
   #endif
   ...
   add_field2(season_section,"canopy_cover_total_final"  ,CSVC_canopy_cover_total_final,UC_fraction,UT_arbitrary_date); //200127
   add_field2(season_section,"thermal_time"   ,CSVC_crop_base_seasonal_thermal_time_period_sum,UC_Celcius_degree_days,UT_arbitrary_date); //200722
}
//_Season_data_source:constructor___________________________________2004-09-21_/
void Season_data_source::record(datetime64 date_time)
{  set_date(date_time);
   set((*this));
   clear();                                                                      //110117
}
//_record______________________________________________________________________/
