...
#include "evaporation_vapor_path.h"
#include "soil/soil_base.h"
#include <corn/math/moremath.h>
//---------------------------------------------------------------------------
#pragma package(smart_init)

/*_______*/ Soil_evaporation_with_vapor_path::Soil_evaporation_with_vapor_path
/*_______*/ (
...
/*_______*/ , soil_layers                       (_soil_layers)
...
/*_______*/    clear_layer_array64(vapor_path_length_upper);   // meter?
/*_______*/    clear_layer_array64(vapor_path_length_lower);   // meter?
/*_______*/ };
//______________________________________________________________________________
/*_______*/ void Soil_evaporation_with_vapor_path::initialize(uint8 number_real_plus_extended_layer)        not_const
/*_______*/ {
...
/*090417_*/       = (FC_1 - PWP_1 / 3.0) * soil_thickness_1_m * water_density;
...
/*090417_*/       = (WC_1 - PWP_1 / 3.0) * soil_thickness_1_m * water_density;
...
/*_______*/ };
//______________________________________________________________________________
/*_______*/ float64 Soil_evaporation_with_vapor_path::evaporate                                    // Sub CalculateSoilWaterEvaporation
...
/*090417_*/    {  float64 soil_thickness_1_m = soil_layers.get_thickness_m(1);
/*090417_*/       float64 top_layer_water_depth_gain
/*090417_*/          = (new_WC - old_WC) * soil_thickness_1_m * water_density;                     //    Top_Layer_water_Depth_Gain = (New_WC - Old_WC) * Soil.LayerThickness(1) * 1000
/*090417_*/       max_current_period_evaporation -= sum_evaporation_mm_ts + top_layer_water_depth_gain;  //    Max_Current_Period_Evaporation = Max_Current_Period_Evaporation - Sum_Evaporation + Top_Layer_water_Depth_Gain
...
#ifdef OBSOLETE
090417 replaced with above
/*_______*/    float64 actual_evaporation_mm_ts = calc_actual(time_step, hour,pot_evap_mm_day);    // Actual_Evaporation = ActualEvaporation(Atmospheric_Water_Potential, Time_Step, Hour, Soil, Weather) 'Return in mm per time step
/*_______*/
/*_______*/    // Note that pot_evap_mm_day should be set to  0 if no snow cover
/*_______*/    // or ponding
/*_______*/    // I have check that CropSyst does set this to 0 for snow cover,
/*_______*/    // but not yet ponding.
/*_______*/
/*_______*/    // In the C++ code we provide water depth in mm/timstep
///*_______*/    float64 water_depth_in_mm_per_ts = flux_mm * time_step;                           // Water_Depth_In = Flux * Time_Step 'Convert Flux (kg/m2/s) to water depth equivalent for the time step (mm)
/*_______*/    if (water_depth_in_mm_per_ts > 0)                                                   // If Water_Depth_In > 0 Then
/*_______*/    {  if (actual_evaporation_mm_ts > water_depth_in_mm_per_ts)                         //     If Actual_Evaporation > Water_Depth_In Then
/*_______*/       {  evaporation_to_subtract_mm_ts = actual_evaporation_mm_ts - water_depth_in_mm_per_ts; //         Evaporation_To_Substract = Actual_Evaporation - Water_Depth_In
/*_______*/          water_depth_in_mm_per_ts = 0.0;                                               //         Water_Depth_In = 0
/*_______*/       } else                                                                           //         Else
/*_______*/       {  water_depth_in_mm_per_ts -= actual_evaporation_mm_ts;                         //         Water_Depth_In = Water_Depth_In - Actual_Evaporation
/*_______*/          evaporation_to_subtract_mm_ts = 0.0;                                          //         Evaporation_To_Substract = 0
/*_______*/       };                                                                               //     End If
/*_______*/    } else                                                                              // Else
/*_______*/       evaporation_to_subtract_mm_ts = actual_evaporation_mm_ts;                        //     Evaporation_To_Substract = Actual_Evaporation
/*_______*/                                                                                        // End If
#endif
...
/*_______*/    if (is_zero(water_depth_in_mm_per_ts))                                              // If Flux = 0 Then
/*_______*/    {  for (int i = 1; i <= 2/*3*/; i++)                                                //     For i = 1 To 3
/*_______*/       {  /* Change 3 to 2 if only one fully " dryable" layers are desired     */       //          'Change 3 to 2 if only one fully " dryable" layers are desired
/*_______*/          /* This is added to hydraulic conductance calculation                */       //          'This is added to hydraulic conductance calculation to control evaporation from deep layers. See Function elementflux
/*_______*/          /* to control evaporation from deep layers. See Function elementflux */
/*_______*/          vapor_conductance[i] = calc_vapor_conductance(i);                             //             Soil_Vapor_Conductance(i) = SoilVaporConductance(i)
/*_______*/       } /*for i */                                                                     //     Next i
/*_______*/    } else                                                                              // End If
/*_______*/    {  // In the V.B. code vapor conductance is set to 9999 at the beginning of every time step.
/*_______*/       for (uint8 layer = 1; layer <= 2/*3*/;layer++)
/*_______*/          vapor_conductance[layer] = 9999.0;
/*_______*/    };
...
/*_______*/ };                                                                                     // End Sub
//_evaporate____________________________________________2009-01-06__2009-04-17_/
float64  Soil_evaporation_with_vapor_path
...
cout << "pot_evap_kg_m2_s set to fixed value for debugging" << endl;
      ...
      ? wu_1                                                                                       //    Evap_Layer_WC = wu(1)
                                                                                                   //    Else
      : 0.8 * wu_1 + wl_1 * 0.2; // Could be 0.5 and 0.5                                           //    Evap_Layer_WC = 0.8 * wu(1) + wl(1) * 0.2 'Could be 0.5 and 0.5 as well 'Roger 040909
                                                                                                   //End If
#ifdef OBSOLETE
090417 now only one dryable layer done above
      ...
      actual_evaporation_kg_m2_s =
         CORN::must_be_0_or_greater<float64>(actual_evaporation_kg_m2_s);                          //If Actual_Evaporation < 0 Then Actual_Evaporation = 0
...
void Soil_evaporation_with_vapor_path
...
//cout << "WARNING setting fixed threshold_WC for debugging" << endl;
   ...
   {  float64 water_depth_available_to_evaporate   /*in mm?*/                                      //    Water_Depth_Available_To_Evaporate
      ...
      vapor_path_length_upper[i] =  (wu_i < threshold_WC)                                          //    If wu(i) < Threshold_WC Then
                                                                                                   //        Vapor_Path_Length_Upper(i)                         _
            ? half_layer_thickness_m                                                               //         = Half_Layer_Thickness  _
               * (1.0 - pow(((wu_i - air_dry_water_content)                                        //            * (1 - ((wu(i) - Air_Dry_Water_Content)                _
                      / (threshold_WC - air_dry_water_content)) , 0.5))                            //                   / (Threshold_WC - Air_Dry_Water_Content)) ^ 0.5)
            :                                                                                      //        Else
              0.0;                                                                                 //        Vapor_Path_Length_Upper(i) = 0
                                                                                                   //    End If
       vapor_path_length_upper[i] = CORN_must_be_between_f64                                       //    If Vapor_Path_Length_Upper(i) > Half_Layer_Thickness Then _
            (vapor_path_length_upper[i],0.00001, half_layer_thickness_m                            //      Vapor_Path_Length_Upper(i) = Half_Layer_Thickness
            );                                                                                     //    If Vapor_Path_Length_Upper(i) <= 0 Then Vapor_Path_Length_Upper(i) = 0.00001
      float64 water_content_change_due_to_soil_water_evaporation                                   //    Water_Content_Change_Due_To_Soil_Water_Evaporation _
       ...
       vapor_path_length_upper[i] = half_layer_thickness_m;
   ...
   {  float64 water_depth_available_to_evaporate =  /* in mm */                                    //    Water_Depth_Available_To_Evaporate = (wl(i) - Air_Dry_Water_Content) _
      ...
      vapor_path_length_lower[i] = (wl_i < threshold_WC )                                          //    If wl(i) < Threshold_WC Then
         ?     half_layer_thickness_m *                                                            //        Vapor_Path_Length_Lower(i) = Half_Layer_Thickness * _
               (1.0 - pow(((wl_i - air_dry_water_content)                                          //            (1 - ((wl(i) - Air_Dry_Water_Content)           _
                         / (threshold_WC - air_dry_water_content)) , 0.5))                         //                  / (Threshold_WC - Air_Dry_Water_Content)) ^ 0.5)
        :                                                                                          //        Else
          0.0;                                                                                     //        Vapor_Path_Length_Lower(i) = 0
                                                                                                   //    End If
       vapor_path_length_lower[i] = CORN_must_be_less_or_equal_to_f64                              //    If Vapor_Path_Length_Lower(i) > Half_Layer_Thickness Then _
          (vapor_path_length_lower[i] , half_layer_thickness_m);                                   //       Vapor_Path_Length_Lower(i) = Half_Layer_Thickness
       ...
       vapor_path_length_lower[i] = half_layer_thickness_m;
}                                                                                                  //End Sub
