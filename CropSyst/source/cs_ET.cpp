...
,ET_model_selected                                          (preferred_ET_model) //200511
,wind_speed                                       (weather_provider_.wind_speed) //151022
,vapor_pressure_deficit_fullday_avg
                          (weather_provider_.vapor_pressure_deficit_fullday_avg) //151022
{}
...
{  return must_be_less_or_equal_to<float64>
...
#error reached I think obsolete
...
{  // Note this is a special form for the PT calculation
   ...
   if (CORN::is_zero<float64>(wind_speed_md/*200127 ,0.0,0.000001*/))            //151208
...
float64 CropSyst_ET::calc_pot_ref_evapotranspiration_mm()              mutation_ //180822
...
//             I think this may be correct, but check with Claudio
   ...
   /*200320 Now the user selection is always used
   ET_model_selected =
      ...
      : ET_Priestley_Taylor;
   */
   if (ET_model_selected == ET_Priestley_Taylor)
      std::clog
      << "warning: Priestley-Taylor model deprecated (select Penman-Monteith)"
      << std::endl;
...
const CORN::Quality &CropSyst_ET::update()                        rectification_
{ // Update annual CO2 conc is done on change on new year.
   ...
   set_mm(selected_ET_mm,calculated_quality); //200227 ET_ref[ET_model_selected]);                             //180404
