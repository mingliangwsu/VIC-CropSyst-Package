...
//_Geocoordinate:constructor________________________________________2012-03-25_/
Geocoordinate::Geocoordinate
(float32 latitude_dec_deg_
,float32 longitude_dec_deg_
,float32 elevation_)
: coordinate                                                                  () //190923
, ref_preferred_format(0)                                                        //140128
{  latitude_dec_deg = latitude_dec_deg_;
   longitude_dec_deg = longitude_dec_deg_;
   elevation = elevation_;                                                       //150817LML
}
//_Geocoordinate:constructor________________________________________2012-03-25_/
...
//_Geocoordinate:constructor_copy___________________________________2012-03-25_/
...
//_Geocoordinate:constructor_from_string_known_format_______________2015-10-26_/
Geocoordinate::Geocoordinate                                                     //151026
(const std::string &from_string)
: coordinate                                                                  () //190923
,ref_preferred_format(0)
...
//_Geocoordinate:constructor_from_string_surmise____________________2015-10-26_/
Geocoordinate::~Geocoordinate() {}
//_Geocoordinate:destructor_________________________________________2013-05-15_/
...
float64 Geocoordinate::decimal_degrees_to_DMS
(float64 decimal_degrees_signed,bool longitude
, int16 &degrees,int16 &minutes, float64 &seconds,char &direction)         const
...
char * Geocoordinate::decimal_degrees_to_DMS_cstr
(float64 decimal_degrees,bool longitude,int decimals, char *buffer)        const
...
std::string &Geocoordinate::decimal_degrees_to_DMS_string
(float64 decimal_degrees,bool longitude,int decimals
,std::string &append_buffer)                                               const
{  // DMS will be formated D°M'S"d and will be appended to the buffer
   char buffer[100]; buffer[0] = 0;
   append_buffer.append(decimal_degrees_to_DMS_cstr
      (decimal_degrees, longitude, decimals, buffer));
   return append_buffer;
}
//_decimal_degrees_to_DMS_string____________________________________2010-01-07_/
...
//_get_longitude_radians____________________________________________1999-10-04_/
...
(float32 degrees,float32 minutes, float32 seconds)                               //050526
{  return set_longitude_dec_deg_f32
      (DMS_to_decimal_degrees(degrees,minutes,seconds));}
//______________________________________________________________________________
...
,char  separator                    /*optional*/)                          const
{
   ...
   */
   ...
   if (separator) to_buffer+=separator;
   //Longitude
...
//_set_latitude_dec_deg_cstr________________________________________2009-09-22_/
float32 Geocoordinate::set_longitude_dec_deg_cstr(const char * _longitude)
{  return set_longitude_dec_deg_f64(atof (_longitude));
}
//_set_longitude_dec_deg_cstr_______________________________________2009-09-22_/
float32 Geocoordinate::set_elevation_meter_cstr(const char * _elevation)
{  return elevation = atof(_elevation);
}
//_set_elevation_meter_cstr_________________________________________2009-09-22_/
float32 Geocoordinate::set_elevation_feet_cstr(const char * _elevation_feet)
{  return elevation = foot_to_m(atof(_elevation_feet));
}
//_set_elevation_feet_cstr_____________________________________________________/
float64 Geocoordinate::set_latitude_DMS_cstr(const char *latitude_)
{  float32 DMS[3] = {0,0,0};
   Coordinate_value coord_value = unknown_value;                                 //120327
   parse_DMS_or_decimal_deg(latitude_,coord_value,DMS);                          //120327
   return set_latitude_DMS(DMS[0],DMS[1],DMS[2]);
}
//_set_latitude_DMS_cstr_______________________________________________________/
float64 Geocoordinate::set_longitude_DMS_cstr(const char *longitude_)
{  float32 DMS[3] = {0,0,0};
   Coordinate_value coord_value = unknown_value;                                 //120327
   parse_DMS_or_decimal_deg(longitude_,coord_value,DMS);                         //120327
   return set_longitude_DMS(DMS[0],DMS[1],DMS[2]);
}
//_set_longitude_DMS_cstr______________________________________________________/
...
,const std::string          &from_string)
{
   ...
   for (std::size_t pos = 0; pos < from_string.length(); pos++)
   {
      ...
      switch (char_at_pos)
      {
         ...
         case ' ' : format.separator = ' ';  break;
         default :
            if (isdigit(char_at_pos) && !in_elevation)
            {
               if (in_decimal) precision     += 1;
               else           leading_digits += 1;
            }
         break;
      } // switch
   }
...
,const Geocoordinate_format &known_format)                         modification_
{
   ...
   context[2][0].integer_digits_max = 9;
   nat8 order_index_ = 0;
   nat8 sign_count = 0;
   for (std::size_t pos = 0; pos < from_string.length(); pos++)
   {
      ...
      switch (char_at_pos)
      {
         ...
         case 'N' :
            ...
            // we've finished latitude now expecting longitude
            order_index_++;
            in_DMSE = 0;
         break;
         case 'S' :
         ...
         case 'E' :
            ...
            // we've finished longitude now expecting elevation
            order_index_++;
            in_DMSE = 0;
         break;
         case 'W' :
            ...
            in_DMSE = 0;
         break;
         case 'm' : // end of elevation                                          //191018
            order_index_++;
            in_DMSE = 0;
         break;
         case '.' :
         ...
         */
         default :
            ...
            {
               bool at_max_integer = context_curr.take_digit(char_at_pos);
