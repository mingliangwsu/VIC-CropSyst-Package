...
bool as_Excel_format = false;
// quick and dirty for debuggin
// need to identify the file type from extension and make available here
...
/*200429
   switch (format.format_file_type_clad.get())                                   //190506
   {
      case FILE_TYPE_text      :
...
*/
      ...
      {  // copy the format column widths to give to tabular_file
         ...
         FOR_EACH_IN
            (format_column,ED_tabular_file_format::Column                        //050411
            ,format.columns //200429 columns_1based
            ,each_format_column)
         {  column_start.append (format_column->delineation.position); //200420 (format_column->position); //100104
            column_widths.append(format_column->delineation.width); //200420 field_width);                    //050411
         } FOR_EACH_END(each_format_column)                                      //050411
         CORN::OS::File_name *foreign_filename_specified                         //200805
            = parameters.foreign_filename_discovery.provide_primary();           //200805
         provided_tabular_file =
            (foreign_filename_specified)                                         //200805
            ? new CORN::Tabular_text_file
               (foreign_filename_specified->c_str()                              //200805_190506
         ...
         provided_tabular_file =
            ...
            ? new CORN::Tabular_text_file
               (parameters.target_filename.c_str()                               //190506
               ,arguments.is_import_mode()                                       //171120
                ? std::ios::in
                : std::ios::out
               ,false
...
*/
      if (as_Excel_format)
      {
         ...
         // weather importing or exporting Excel, the filename must be specified
         CORN::OS::File_name *Excel_filename =                                   //200805
            parameters.foreign_filename_discovery.provide_primary();            //200805
         if (Excel_filename)                                                     //200805
         {  // The excel filename must be specified
         if (import_mode)                                                        //101127
            cell_reader = new Cell_reader_Excel_XLS
               (Excel_filename->c_str());                                        //200805
               //200805 (parameters.target_filename.c_str()); //190506_110828_100113
         else                                                                    //101127
            provided_tabular_file = new CORN::Tabular_file_Excel                 //061018
               (Excel_filename->c_str()                                          //200806
               //200805 parameters.target_filename.c_str()                               //190506
      ...
      {  CORN::OS::File_name *foreign_filename
            = parameters.foreign_filename_discovery.provide_primary();           //200805
         const char *foreign_filename_cstr
            = foreign_filename ? foreign_filename->c_str() : 0;                  //200805
         cerr << "Unable to determine the file type or open a data source for the file:"
              << foreign_filename_cstr << endl;
...
void Text_dBase_Excel_with_TDF_convertor::recognize_current_sheet_name
(const std::string &encountered_sheet_name)                        modification_
{
...
{  bool used = true;
   nat32 cell_row_num = cell->get_row()+1;
   nat32 cell_col_num = cell->get_col()+1;
   ...
   const ED_tabular_file_format::Metadata_field *metadata_field
     = format.find_metadata_field(current_sheet_name,cell_row_num,cell_col_num);
   if (metadata_field)
   {
...
bool Text_dBase_Excel_with_TDF_convertor::assign_numeric_detail
(Cell_numeric *cell)                                               modification_
// because receiver may modify self
{  bool used = true;
   nat32 cell_row_num = cell->get_row()+1;
   commit_detail_line_record_if_new_row_encountered(cell_row_num);
   nat32  actual_detail_line_start_row_num = format.detail.extent.begin_offset + 1; //200429  I think +1 is now needed to get number
      //200429 detail_lines_delineation.start_row_number;
   // WARNING currently simply assuming the start_row_num is relative to the top of the of the worksheet
   // (assuming no begin marker etc.)
   if (cell_row_num >=actual_detail_line_start_row_num)
...
{  bool used = false;
   ...
   // scan the columns if the cell is a data column and the row is detail lines
   // For Excel it is valid to assume cells are provided in row major order
   nat32 actual_detail_line_start_row_num
...
{  nat32 metadata_fields = 0;
   ...
   {  int last_header_row = 0;
      ...
      FOR_EACH_IN(metadata_field                                                 //100107
         ...
         ,format.metadata_fields_numbered,each_header)
      {  // Write the caption
         ...
         {  size_t caption_length =  metadata_field->caption.length();
            ...
            switch (metadata_field->field_data_clad.get())
            {  case ED_tabular_file_format::Metadata_field::FD_static_text :
               ...
               {  // get from UED database description record with database_comment_index
                  ...
                  CORN::Item_string *primary = dynamic_cast<CORN::Item_string *>(comments.get_at(0));
                  if (primary)
               ...
               {  // get from UED database description record with database_description_index
                  ...
                  CORN::Item_string *primary = dynamic_cast<CORN::Item_string *>(descriptions.get_at(0));
                  if (primary)
                     sheet->set_cell_label(metadata_field->col_num_not_pos-1,metadata_field->row_num-1,*primary);
               } break;
               ...
               {  const UED::Geolocation_record *geoloc_rec = UED_file->ref_geolocation_record();
                  ...
                  {  char value_buffer[50]; value_buffer[0] = 0;
                     ...
                     switch (metadata_field->location_var_clad.get())
                     {  case ED_tabular_file_format::Metadata_field::LOC_VAR_station_ID                : sheet->set_cell_label    (metadata_field->col_num_not_pos-1,metadata_field->row_num-1,geoloc_rec->get_station_ID_code());                                         break;
                        case ED_tabular_file_format::Metadata_field::LOC_VAR_latitude_decimal_degrees  : sheet->set_cell_number   (metadata_field->col_num_not_pos-1,metadata_field->row_num-1,(float64)geoloc_rec->get_latitude_dec_deg(), metadata_field->format.precision/*200429  metadata_field->precision*/);   break;
                        case ED_tabular_file_format::Metadata_field::LOC_VAR_latitude_deg_min_sec      :
                        {  geoloc_rec->decimal_degrees_to_DMS_string(geoloc_rec->get_latitude_dec_deg(),false,metadata_field->format.precision/*200429  metadata_field->precision*/,value_buffer_string);
                           sheet->set_cell_label    (metadata_field->col_num_not_pos-1,metadata_field->row_num-1,value_buffer_string);
                        } break;
                        case ED_tabular_file_format::Metadata_field::LOC_VAR_longitude_decimal_degrees : sheet->set_cell_number   (metadata_field->col_num_not_pos-1,metadata_field->row_num-1,(float64)geoloc_rec->get_longitude_dec_deg(), metadata_field->format.precision/*200429  metadata_field->precision*/);   break;
                        case ED_tabular_file_format::Metadata_field::LOC_VAR_longitude_deg_min_sec     :
                        {  geoloc_rec->decimal_degrees_to_DMS_string(geoloc_rec->get_longitude_dec_deg(),true,metadata_field->format.precision/*200429  metadata_field->precision*/,value_buffer_string);
                           sheet->set_cell_label    (metadata_field->col_num_not_pos-1,metadata_field->row_num-1,value_buffer_string);
                        } break;
                        case ED_tabular_file_format::Metadata_field::LOC_VAR_elevation_meters          : sheet->set_cell_number   (metadata_field->col_num_not_pos-1,metadata_field->row_num-1,(float64)geoloc_rec->get_elevation(), metadata_field->format.precision/*200429  metadata_field->precision*/);           break;
                        case ED_tabular_file_format::Metadata_field::LOC_VAR_elevation_feet            : sheet->set_cell_number   (metadata_field->col_num_not_pos-1,metadata_field->row_num-1,(float64)m_to_foot(geoloc_rec->get_elevation()),metadata_field->format.precision/*200429  metadata_field->precision*/); break;
...
nat32  Text_dBase_Excel_with_TDF_convertor::export_column_headers()
{
   ...
   {  CORN::Workbook &workbook = tabular_Excel->get_workbook();
       ...
       //      tabular_file->goto_raw_line(format.column_headers_delineation.start_row_number);
      ...
      {  std::string sheet_name(column-> sheet.name); if (!sheet_name.length()) sheet_name.assign("Sheet 1");
         ...
         if (sheet)
         {  CORN::SDF_Clad/*SDF_List*/ caption_SDF_list(column->get_caption_SDF());
            for (int8 row_idx = format.header.extent.rows-1                      //200429
                  //200429 column_headers_delineation.rows-1
                ;row_idx >=0
                ;row_idx--)
            {   // For each column header, take the description (caption) and split the text into words
                ...
                CORN::Item_string *caption_fragment = dynamic_cast<CORN::Item_string *>
                  (caption_SDF_list.pop_tail());
                if (!caption_fragment) caption_fragment = new CORN::Item_string("");
                if (row_idx == 0)
                ...
                sheet->set_cell_label
                  (column->delineation.position-1 /*200429  position-1*/
                  //200429 ,format.column_headers_delineation.start_row_number
                  ,format.header.extent.begin_offset                             //200429
                     +row_idx-1,cell_label);
             } // for row_idx
          } //if sheet
       } FOR_EACH_END(each_col_desc)
       //200429 if (format.column_headers_delineation.units_rows)                         //070307
       if (format.header.units_rows)                                             //200429_070307
       {  // Output column header units description
         ...
         FOR_EACH_IN(column,ED_tabular_file_format::Column
            //200429 ,format.columns_1based
            ,format.columns                                                      //200429
            ,each_col_desc)
         {  std::string sheet_name(column->sheet.name); if (!sheet_name.length()) sheet_name.assign("Sheet 1");
            CORN::Workbook::Sheet *sheet = workbook.provide_sheet(sheet_name);
            if (sheet)
            {  std::string units = column->get_units_caption();  // preferred units description if specified
               ...
               UED::Variable_code_cowl                                           //170327
                  &smart_var_code = column->variable_code_clad;
   //             if (format.column_tabulation.get() == fixed_TABULATION)   //090712
   ...
   //             }
   //             if (delimiter && curr_col_number > 1)                          //090712
   //                tabular_file->write_delimiter_string(delimiter);
               if (smart_var_code.is_layered() && column->spread/*200429 spread_columns*/)
                  for (int layer = 0; layer < column->spread/*200429 spread_columns*/; layer++)
                     sheet->set_cell_label(column->delineation.position-1
                        ,(format.header.extent.begin_offset)                     //200429 now sure if still -1
                         +format.header.units_row_offset,units);                 //200429

                     /*200429
                     sheet->set_cell_label(column->position-1
                        ,(format.column_headers_delineation.start_row_number-1)
                         +format.column_headers_delineation.units_row_offset,units);
                     */
               else
                  sheet->set_cell_label
                     (column->delineation.position-1                             //200429
                     ,(format.header.extent.begin_offset)                        //200429
                     + format.header.units_row_offset,units);                    //200429
                  /*200429
                  sheet->set_cell_label(column->position-1
      ...
      exported_col_header_rows = format.header.extent.rows;                      //200429
         //200429   column_headers_delineation.rows;
...
//_export_column_headers____________________________________________2011-12-12_/
