...
//140222   if (!project_parameters) return;    // Just incase the project parameters are not provided
   ...
   {  nat32 tally_var_code =
         ( annual_tally ?  YLY_report_var_opts[report_options_index].ID : GS_report_var_opts[report_options_index].ID);
            const char *variable_description_abbrv = Cropsyst_variable_code_clad(tally_var_code).get_abbreviation();
...
//_Avg_prob_map_tallier_____________________________________________2009-10-30_/
...
//_expect_structure_________________________________________________2005-05-05_/
...
//_Tally_variable___________________________________________________2005-07-05_/
...
//_clear____________________________________________________________2005-05-05_/
...
//_recompute________________________________________________________2005-05-05_/
...
//_append_value_________________________________________2005-07-11__2005-11-10_/
...
//_set_start________________________________________________________2005-05-07_/
...
//   Ustring status(buffer);
   std::string status(Cropsyst_variable_code_clad(variable_code).get_abbreviation());
...
//_tally_UED_file______________________________________________________________/
...
int32 get_largest_file_num(CORN_filename &pattern)                               //140203
{
  ...
  //   largest = max(largest,filenumber);
   ...
   {  if (isdigit(file_block.ff_name[0]))  // should provide a more robust check if the filename is a number
      {  int32 file_num = atol (file_block.ff_name);                             //140203
...
//_get_largest_file_num________________________________________________________/
int get_tally_column(CORN_filename filename)
{
   ...
   int num_cols = 0;
   BIFF234_File SS_file(filename.c_str());                                          //990607
   if (SS_file.good())
   {  while (!SS_file.eof())
      { SS_file.read_record();
         if (( SS_file.data_type == Spreadsheet_abstract::type_str) &&
...
//_get_tally_column____________________________________________________________/
char  extension[10];
//______________________________________________________________________________
...
//_duplicate_scenario_record________________________________________2005-09-20_/
