...
:Residues_common
   ...
   ,temperature_air_avg_,wind_speed_)                                            //060330
...
{}
//_Residues_V4_common:constructor____________________________________________020307__/
...
//_update_totals_sum_pools_____________________________________________________/
float64 Residues_V4_common::get_biomass_output
...
//_get_surface_all_biomass_____________________________________________________/
...
//_get_surface_plant_biomass___________________________________________________/
float64 Residues_V4_common::get_subsurface_all_biomass()                   const
{  return organic_matter_pools.get_mass(BIOMASS_TOTAL                            //060331
      ,(INCLUDE_STRAW_RESIDUE_OM|INCLUDE_MANURE_RESIDUE_OM)
      ,INCLUDE_POSITION_SUBSURFACE);
}
//_get_subsurface_all_biomass_______________________________________2006-01-16_/
...
//_get_total_surface_biomass________________________________________2001-10-11_/
float64 Residues_V4_common::get_subsurface_XXXX_biomass
(soil_sublayer_array_64(layer_residues),Organic_matter_type residue_type)
...
//_get_subsurface_XXXX_biomass______________________________________2000-08-10_/
...
//_balance__________________________________________________________2008-07-18_/
...
,float64 fract_surface_to_shallow
,float64 shallow_horizon_thickness_                                              //990217
,float64 deep_horizon_thickness_)                                                //990217
{  // Check if there are indeed any deep horizons
   ...
   if (fract_to_shallow > 0.0)                                                   //060605
   {
      ...
      to_shallow->know_contact_fraction(subsurface_contact_fraction);            //190327_060330
   }
   if (fract_to_deeps > 0.0)                                                     //060605
   {                                                                             //060330
      to_deeps = pool_to_redistributed->clone();                                 //111001
      to_deeps->multiply_by(fract_to_deeps);                                     //060330
      to_deeps->know_contact_fraction(subsurface_contact_fraction);              //190327
   }
   // Deduct
   ...
   {  Organic_biomatter_common *to_this_layer = 0;
      {  float64 layer_thickness = soil_layers.get_thickness_m(layer);
         ...
         {  case SHALLOW_HORIZON :
            ...
            {  fract_apportion_this_layer
                  = layer_thickness / shallow_horizon_thickness_;
               to_this_layer = to_shallow->clone();                              //111001
            } break;
            case DEEP_HORIZON : // case  DEEP_HORIZON      :
            if (to_deeps)                                                        //060605
            {  fract_apportion_this_layer
               = layer_thickness / deep_horizon_thickness_;
         ...
         {  to_this_layer->multiply_by(fract_apportion_this_layer);
            ...
            to_this_layer->know_contact_fraction(subsurface_contact_fraction);   //190327_060605
            //190327to_this_layer->set_contact_fraction(subsurface_contact_fraction);    //060605
...
{  // This method may return null if not usable biomass is available for disposal
   ...
   {  if (curr_pool->is_above_ground())
      {  removed_biomass_amount +=  curr_pool->kg() * fract_to_remove;
         #ifdef DETAILED_BALANCES
         //130711 we dont bother with the detailed balance check
         // (actually we should have a flag if we want the residue balance at all
         if (residue_mass_balance[curr_pool->position])                          //090408
             residue_mass_balance[curr_pool->position]->removed
               += removed_biomass_amount;                                        //090407
         #endif
...
May in the future need to have a biomass fate, but even then we are just removing it from the system
   ...
   removed_biomass->set_contact_fraction
      (DEFAULT_SURFACE_ORGANIC_RESIDUE_CONTACT_FRACTION);                        //040818
...
//_get_pool_list____________________________________________________2006-02-02_/
void Residues_V4_common::decompose_all_without_N()
{
...
//_decompose_all_without_N__________________________________________2004-06-08_/
...
//_calc_carbon_nitrogen_ratio__________________________________________________/
              // Eventually make this function a member
float64 calc_cumulative_residue_incorporation_fraction
(float64 layer_bottom_depth, float64 tillage_depth)
...
//_calc_cumulative_residue_incorporation_fraction___________________2006-07-28_/
...
//cout << "RMB:" << get_surface_all_biomass() << '\t' << get_subsurface_all_biomass() << endl;
   FOR_EACH_IN(potential_standing_stubble_residue_pool,Organic_biomatter_common,organic_matter_pools,each_standing_stubble) //090309
   {
      ...
      {  Organic_biomatter_common *stubble =  potential_standing_stubble_residue_pool;
         int RUSLE2_residue_type_index =  stubble->get_RUSLE2_residue_type() - 1;
         // get_RUSLE2_residue_type() ranges from 1 (0 unknown is not used)
         // but   residue_flatten_ratio is 0 index //090311
         if (RUSLE2_residue_type_index < 0) RUSLE2_residue_type_index = 0;
         // If the crop did not specify the residue type, just use the first one //090312
...
// if needed create a new residue pool correctly assocated with the organic matter pool for the layer
         ...
         (flat_surface_residue_pool->is_flat_surface()))
      {
         ...
         int RUSLE2_residue_type_index =  flat_surface_residue_pool->get_RUSLE2_residue_type() - 1;
            // get_RUSLE2_residue_type() ranges from 1 (0 unknown is not used)
            // but   residue_flatten_ratio is 0 index                            //090311
         if (RUSLE2_residue_type_index < 0) RUSLE2_residue_type_index = 0;
         // If the crop did not specify the residue type, just use the first one //090312
         ...
         for (nat8 layer = 1; layer <= last_tilled_layer; layer++)
         {
            if (surface_residue_mass_fraction_into[layer] > 0.0)
            {
               ...
               {  setup_contribution(*residue_from_surface_to_layer);            //060728
                  residue_from_surface_to_layer->set_position(OM_subsurface_position);
                  ...
                  if (residue_mass_balance[OM_subsurface_position])    // Note, for subsurface, incorporated is what is added from the surface  //090408
                      residue_mass_balance[OM_subsurface_position]->incorporated
                      += residue_from_surface_to_layer->kg();                    //090407
                  #endif
               }
...
//_respond_to_field_operation_______________________________________2006-07-28_/
...
//_Residues_V4_common::find_________________________________________2006-07-27_/
