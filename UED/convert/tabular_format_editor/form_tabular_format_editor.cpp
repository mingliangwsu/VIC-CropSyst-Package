...
void Ttabular_format_editor_form::bind_to(ED_tabular_file_format *i_format,Tparameter_file_form *_parameter_file_form)
{
   ...
   // Information
   defined_by_edit                  ->bind_to(&(format->definer         ),NO_HELP_YET);
   defined_date_button              ->bind_to(&(format->inception)       ,"Definition date (usually today)",false,NO_HELP_YET);
...
//050321 Add here end_data_maker  (character string)
   ...
   //Header fields
   {
      ...
      header_fields_listbox            ->bind_to(&(format->metadata_fields_numbered            ),NO_HELP_YET);
   ...
   columns_listbox                  ->bind_to(&(format->columns_numbered                  ),NO_HELP_YET);
...
// column_field_form->bind_to(xxx); can't bind until selected
   ...
   edit_ID_column->bind_to(&(format->ID_column_number),NO_HELP_YET);
   edit_name_column->bind_to(&(format->name_column_number),NO_HELP_YET);
/*
...
{  if (!format) return;
   ...
   if (field_to_add)
      format->metadata_fields_numbered.append(field_to_add); // relinquish to format->header_fields_numbered
   header_fields_listbox->Update();
   int metadata_field_count = format->metadata_fields_numbered.count();
...
{  ED_tabular_file_format::Metadata_field *field_to_edit = (ED_tabular_file_format::Metadata_field *)
   format->metadata_fields_numbered.get_at(header_fields_listbox->ItemIndex);
   if (field_to_edit)
...
{  ED_tabular_file_format::Column *added_column = new ED_tabular_file_format::Column;
   if (added_column)
      format->columns_numbered.append(added_column);
   columns_listbox->Update();
   int column_count =  format->columns_numbered.count();
...
void __fastcall Ttabular_format_editor_form::columns_listbox_onclick(TObject *Sender)
{
   ...
   format->columns_numbered.get_at(columns_listbox->ItemIndex);
   if (column_to_edit)
   {
...
{
   format->ID_column_number_numbered = columns_listbox->ItemIndex+1;
   edit_ID_column->Update();
   edit_name_column->Update();
}
//---------------------------------------------------------------------------
...
{
   format->name_column_number_numbered = columns_listbox->ItemIndex+1;
   edit_name_column->Update();
}
//---------------------------------------------------------------------------
