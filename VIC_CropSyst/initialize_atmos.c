...
/**********************************************************************
  ...
  2012-Aug-07 Fixed bug in handling (QAIR or REL_HUMID) + PRESSURE supplied, in
          ...
          switched.                                TJB
  2012-Nov-13 stored min/max relative humidity value into the code to be
            passed to CropSyst          KM
**********************************************************************/
{
  ...
  #if (VIC_CROPSYST_VERSION>=3)
  double *rhum_max;                                                              //LML 150424 (%)
  double *rhum_min;                                                              //LML 150424 (%)
  #endif
  ...
  *******************************/
  ...
  forcing_data = read_forcing_data(infile);
      ...
      if (param_set.TYPE[type].SUPPLIED) {
        ...
        if (   type == PREC
          ...
          for (idx=0; idx<(global_param.nrecs*NF); idx++) {
            ///forcing_data[type][idx] *= global_param.dt * 3600; keyvan changed to be the same as latest version to fix bugs 130713
  ...
  ******************************************************/
  ...
  if(!(param_set.TYPE[TMAX].SUPPLIED && param_set.TYPE[TMIN].SUPPLIED)) {
    for (day=0; day<Ndays_local; day++) {
      ...
      for (hour = 0; hour < 24; hour++) {
        if ( hour >= 9 && ( tmax[day] == -9999
                            || local_forcing_data[AIR_TEMP][day*24 + hour] > tmax[day] ) )
          tmax[day] = local_forcing_data[AIR_TEMP][day*24 + hour];               //LML 1504224 added day*24
        if ( hour < 12 && ( tmin[day] == -9999
                            || local_forcing_data[AIR_TEMP][day*24 + hour] < tmin[day] ) )
          tmin[day] = local_forcing_data[AIR_TEMP][day*24 + hour];               //LML 1504224 added day*24
      }
    }
  }
    ...
    for (day=0; day<Ndays_local; day++) {
      for (hour=0; hour<24; hour++) {
        if(param_set.FORCE_DT[param_set.TYPE[SHORTWAVE].SUPPLIED-1] == 24) {
          ...
          //std::clog << "day:" << day
          //          << "\thour:" << hour
          //          << "\thourlyrad:" << hourlyrad[day*24+hour]
          //          << std::endl;
  ...
  if(!param_set.TYPE[AIR_TEMP].SUPPLIED) {
    ...
    **********************************************************************/
    ...
    for(rec = 0; rec < global_param.nrecs; rec++) {
      ...
      if(NF>1) atmos[rec].air_temp[NR] = sum / (float)NF;
/**********************************************************************
    Set daily tmax and tmin to the variables in the atmos_data_structure -08102010 KJC
**********************************************************************/
#if VIC_CROPSYST_VERSION==2
      atmos[rec].tmax/*[NR]*/ = tmax[rec];/* 130215 keyvan*/ /*to get more info about NR and NF check get_global_param.c line 708-717*/
      atmos[rec].tmin/*[NR]*/ = tmin[rec];/*130215 keyvan*/
#endif
    }
  }
  ...
  if(!param_set.TYPE[PRESSURE].SUPPLIED) {
    if(!param_set.TYPE[DENSITY].SUPPLIED) {
      ...
      if (options.PLAPSE) {
        ...
        for (rec = 0; rec < global_param.nrecs; rec++) {
          atmos[rec].pressure[NR] = PS_PM*exp(-elevation*GRAVITY/(Rd*(KELVIN+atmos[rec].air_temp[NR]+0.5*elevation*LAPSE_PM)));
          for (i = 0; i < NF; i++) {
            atmos[rec].pressure[i] = PS_PM*exp(-elevation*GRAVITY/(Rd*(KELVIN+atmos[rec].air_temp[i]+0.5*elevation*LAPSE_PM)));
          }
        }
      }
      else {
...
#ifdef VIC_CROPSYST_VERSION
            sum_sph += local_forcing_data[QAIR][idx];
#endif
            sum += atmos[rec].vp[j];
          }
          if(NF>1) {
...
#ifdef VIC_CROPSYST_VERSION
              atmos[rec].VCS.sph[NR] = sum_sph / (float)NF;
#endif
          }
        }
      }
      ...
      if(param_set.FORCE_DT[param_set.TYPE[REL_HUMID].SUPPLIED-1] == 24) {
        ...
        for (rec = 0; rec < global_param.nrecs; rec++) {
          ...
          for (j = 0; j < NF; j++) {
            ...
            if (global_param.starthour - hour_offset_int < 0) hour += 24;
            idx = (int)((float)hour/24.0);
            atmos[rec].vp[j] = local_forcing_data[REL_HUMID][idx] * svp(atmos[rec].air_temp[j]) / 100; //LML 150424 this idx is confusing
            sum += atmos[rec].vp[j];
          }
          if(NF>1) atmos[rec].vp[NR] = sum / (float)NF;
        }
  ...
  *************************************************/
  ...
  for(rec = 0; rec < global_param.nrecs; rec++) {
    ...
    for(i = 0; i < NF; i++) {
      ...
      sum2 += atmos[rec].vp[i];
      #if (VIC_CROPSYST_VERSION>=3)
      atmos[rec].VCS.relative_humidity[i] = 100. * atmos[rec].vp[i]/(atmos[rec].vp[i]+atmos[rec].vpd[i]);   //LML 150424
      sumrhum += atmos[rec].VCS.relative_humidity[i];
      #endif
    }
    ...
    #if (VIC_CROPSYST_VERSION>=3)
    //171115LML atmos[rec].VCS.relative_humidity[NR] = 100. * atmos[rec].vp[NR]/(atmos[rec].vp[NR]+atmos[rec].vpd[NR]);  //LML 150424
    if (NF > 1) atmos[rec].VCS.relative_humidity[NR] = sumrhum / (float)NF;                                             //171115LML
    #endif
  }

#if (VIC_CROPSYST_VERSION==2) //Keyvan I'm not sure why we have to have this here,
   for(rec=0; rec<global_param.nrecs; rec++){
     atmos[rec].relative_humidity_min = 9999;
     atmos[rec].relative_humidity_max = -9999;
     for (i=0; i<NF; i++)
     {
       atmos[rec].relative_humidity[i]= 100. * atmos[rec].vp[i]/(atmos[rec].vp[i]+atmos[rec].vpd[i]);
   /* if (i == 0) {
        atmos[rec].relative_humidity_min = atmos[rec].relative_humidity[i];
        atmos[rec].relative_humidity_max = atmos[rec].relative_humidity[i];
    }
    else*/

       if(atmos[rec].relative_humidity[i]<atmos[rec].relative_humidity_min)
         atmos[rec].relative_humidity_min = atmos[rec].relative_humidity[i];
       if(atmos[rec].relative_humidity[i]>atmos[rec].relative_humidity_max)
         atmos[rec].relative_humidity_max = atmos[rec].relative_humidity[i];
   ...
   //LML 150424 set daily Max and Min temperature & relative humidity
   #if VIC_CROPSYST_VERSION>=3
   /*170410LML
   //Calculate daily max & min relative humidity
   for (day=0; day<Ndays_local; day++) {
     rhum_max[day] = rhum_min[day] = -9999;
     for (hour = 0; hour < 24; hour++) {
       double current_rhum = 0.0;
       double current_tair = 0.0;
       if (param_set.TYPE[AIR_TEMP].SUPPLIED &&
           param_set.FORCE_DT[param_set.TYPE[AIR_TEMP].SUPPLIED-1] != 24)
         current_tair = local_forcing_data[AIR_TEMP][day*24 + hour];
       else
         current_tair = tair[day*24 + hour];
       if (param_set.TYPE[REL_HUMID].SUPPLIED) {
         if (param_set.FORCE_DT[param_set.TYPE[REL_HUMID].SUPPLIED-1] == 24) idx = day;
         else idx = day*24 + hour;
         current_rhum = local_forcing_data[REL_HUMID][idx];
       } else {
         if (param_set.TYPE[VP].SUPPLIED
             && param_set.FORCE_DT[param_set.TYPE[VP].SUPPLIED-1] == 24) idx = day;
         else idx = day*24 + hour;
         current_rhum = 100.0 * local_forcing_data[VP][idx]
                                / svp(current_tair);
         current_rhum = CORN::must_be_between<double>(current_rhum,0.0,100.0);
       }
       //printf("hour:%d T:%f\n",hour,current_tair);

       if (day == 0) {
           std::clog << "WARNING: Temporary implementation for calulating max and min rhum!!!\n";
       }
       current_rhum = atmos[day].relative_humidity[hour];

       if (rhum_max[day] == -9999 || current_rhum > rhum_max[day])
         rhum_max[day] = current_rhum;
       if (rhum_min[day] == -9999 || current_rhum < rhum_min[day])
         rhum_min[day] = current_rhum;
     }

     std::clog << "rhum_max:" << rhum_max[day]
               << "rhum_min:" << rhum_min[day]
               << std::endl;

   } */ //170410LML

   //171115LML
   //Set daily max & min relative humidity
   for (day=0; day<Ndays_local; day++) {
     rhum_max[day] = rhum_min[day] = -9999;
     if (param_set.TYPE[RHUM_MAX].SUPPLIED && param_set.TYPE[RHUM_MIN].SUPPLIED)
     {
         if (param_set.FORCE_DT[param_set.TYPE[RHUM_MAX].SUPPLIED-1] == 24 &&
             param_set.FORCE_DT[param_set.TYPE[RHUM_MIN].SUPPLIED-1] == 24) {
             rhum_max[day] = local_forcing_data[RHUM_MAX][day];
             rhum_min[day] = local_forcing_data[RHUM_MIN][day];
         }
     }
   } //170410LML
   ...
   for(rec = 0; rec < global_param.nrecs; rec++) {
       ...
       double rmin = 100;
       for(i = 0; i < 24; i++) {
           if (atmos[rec].VCS.relative_humidity[i] > rmax) rmax = atmos[rec].VCS.relative_humidity[i];
           else if (atmos[rec].VCS.relative_humidity[i] < rmin) rmin = atmos[rec].VCS.relative_humidity[i];
       }
       hour = rec*global_param.dt + global_param.starthour - hour_offset_int;
       if (global_param.starthour - hour_offset_int < 0) hour += 24;
       day = (int)((float)hour/24.0);
       atmos[rec].VCS.tmax = tmax[day];
       atmos[rec].VCS.tmin = tmin[day];
       if (rhum_max[day] > 0 && rhum_min[day] > 0) {
           atmos[rec].VCS.relative_humidity_max = rhum_max[day];
           atmos[rec].VCS.relative_humidity_min = rhum_min[day];
           atmos[rec].VCS.relative_humidity[NF] =
                                          (rhum_max[day] + rhum_min[day]) / 2.0;
       } else {
           atmos[rec].VCS.relative_humidity_max = rmax;
           atmos[rec].VCS.relative_humidity_min = rmin;
       }
   }
   #endif
  ...
  ****************************************************************************/
  ...
  if ( !param_set.TYPE[LONGWAVE].SUPPLIED ) {
    ...
    for (rec = 0; rec < global_param.nrecs; rec++) {
      sum = 0;
      for (i = 0; i < NF; i++) {
        atmos[rec].longwave[i] = calc_longwave(atmos[rec].tskc[i],
