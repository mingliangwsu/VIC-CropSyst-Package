  ...
  for (i = 0; i < ctrl->ndays; i++) {
    ...
    if (ctrl->insw) {
      ...
      for (j=0; j<24; j++) {
        ...
        //std::clog << "i:" << i
        //          << "\tj:" << j
        //          << "\thourlyrad[i*24+j]:" << hourlyrad[i*24+j]
        //          << std::endl;
      }
      ...
      //std::clog << "i:" << i
      //          << "\tmtclim_data->s_srad[i]:" << mtclim_data->s_srad[i]
      //          << std::endl;
...
//#ifdef INPUT_24HR_AVG_SOLAR_RADIATION
//      if (ctrl->insw) {
//        //The default input solar radiation is daytime (daylight) average solor ridiation.
//        //At this case the input is 24-hr average soloridiation, so make the following adjustment
//
//        double adj_24hr_to_daylight = mtclim_data->s_dayl[i] / 86400.0;
//        mtclim_data->s_srad[i] /= adj_24hr_to_daylight;
//
//        //std::clog << "i:" << i
//        //        << "\mtclim_data->s_srad[i]:" << mtclim_data->s_srad[i]
//        //        << "\tadj_24hr_to_daylight:" << adj_24hr_to_daylight
//        //        << std::endl;
//      }
//
//#endif
    ...
    tmp_rad = mtclim_data->s_srad[i] *
//180529LML#ifdef VIC_CROPSYST_VERSION
//180529LML            24.0;                                                                //171115LML for VIC_CROPSYST_VERSION, the salor radiation is daily average, not daytime average.
//180529LML#else
            mtclim_data->s_dayl[i] / 3600.;
//180529LML#endif
