...
:UED_imp_exp_database(database_filename,true,_format)
,export_tuples()
{}
//______________________________________________________________________________
...
void UED_export_database::initialize()
{
  UED_imp_exp_database::initialize();                                            //020512
   ...
   format.metadata_fields_numbered.sort();
}
...
//_get_export_column___________________________________________________________/
...
//_setup_export_columns_____________________________________________2004-09-28_/
#ifdef OLD_STANDALONE
//050901  I am trying now to use   CORN tabular derived classes
//so I can use the new Datasource Data_record model
//Don't delete this until I get Text_dBase_Excel_with_TDF_convertor working
...
/*_______*/    const char *delimiter = col_number == 1 ? "" : format.column_headers_delineation.get_delimiter();
/*_______*/    output_file->export_string(row_substring ,column,format.column_headers_delineation.text_quote_mode_labeled.get(),delimiter);
/*_______*/ };
#endif
//______________________________________________________________________________
#ifdef OLD_STANDALONE
...
/*_______*/     {
/*_______*/        output_file->goto_row(format.column_headers_delineation.start_row_number+CH_row_number-1);
...
/*_______*/     output_file->goto_row(format.column_headers_delineation.start_row_number + format.column_headers_delineation.units_row_offset); // rows);
/*_______*/     FOR_EACH_IN(column,ED_tabular_file_format::Column,format.columns_1based,each_col_desc);
/*_______*/     {
/*_______*/           Ustring units = column->units_caption;  // perferred units description if specified
/*_______*/           if (!units.length() && column->units.get())  // if not specified derived the units description of units_code
/*_______*/              units =
                           column->get_units_caption(); 
                           //101018get_units_caption/*units_description*/(column->smart_units_code_enum.get(),true);
/*_______*/           UED_smart_variable_code &smart_var_code = column->smart_variable_code;
/*050902*/           const char *delimiter = (column_header_number++ == 1)?  "" :format.column_headers_delineation.get_delimiter() ;  // Don't prepend delimiter for column 0
/*_______*/              if (smart_var_code.is_layered() && column->spread_columns)
/*_______*/              {
/*_______*/                 for (int layer = 0; layer < column->spread_columns; layer++)
/*_______*/                 {
/*050901*/                    output_file->export_string(units ,*column,format.column_headers_delineation.text_quote_mode_labeled.get(),delimiter);  // Note the format may reserve multiple rows for units currently only using a single row
/*_______*/                 }
/*_______*/              }
/*_______*/              else
/*_______*/              output_file->export_string(units ,*column,format.column_headers_delineation.text_quote_mode_labeled.get(),delimiter);  // Note the format may reserve multiple rows for units currently only using a single row
/*_______*/        } FOR_EACH_END(each_col_desc)
/*_______*/ }
#endif
//______________________________________________________________________________
...
/*050902*/              const char *delimiter = export_column_number ? format.detail_lines_delineation.get_delimiter() : "";  // Don't prepend delimiter for column 0
...
/*040931*/                          output_file->export_string(tuple_date.text_str(date_str_buffer),export_column->TFFC_layout,format.detail_lines_delineation.text_quote_mode_labeled.get(),delimiter);
/*_______*/                    } break;
/*_______*/                    // NYI case UT_time
/*_______*/                    // NYI case UT_date_time
/*_______*/                    case UT_second:         output_file->export_integer((int16)(tuple_date_time.get_second()),export_column->TFFC_layout,delimiter);  /* date_or_time_printed = true;*/ break;
/*_______*/                    case UT_minute :        output_file->export_integer((int16)(tuple_date_time.get_minute()),export_column->TFFC_layout,delimiter);  /* date_or_time_printed = true;*/ break;
/*_______*/                    case UT_hour:           output_file->export_integer((int16)(tuple_date_time.get_hour()),export_column->TFFC_layout,delimiter);    /* date_or_time_printed = true;*/ break;
/*_______*/                    case UT_day_of_year :   output_file->export_integer((int16)(tuple_date_time.get_DOY()),export_column->TFFC_layout,delimiter);     /* date_or_time_printed = true;*/ break;
...
/*_______*/                       output_file->export_integer((int16)year,export_column->TFFC_layout,delimiter);
//041004 moved above/*_______*/                       date_or_time_printed = true;
/*_______*/                    } break;
/*_______*/                    case UT_day_of_month :  output_file->export_integer((int16)(tuple_date_time.get_DOM()),export_column->TFFC_layout,delimiter);    /* date_or_time_printed = true;*/  break;
/*_______*/                    case UT_month :
/*_______*/                            output_file->export_integer((int16)(tuple_date_time.get_month()),export_column->TFFC_layout,delimiter);  /* date_or_time_printed = true;*/ break;
/*_______*/                 };
/*041004_*/             };
...
/*040931*/                          output_file->export_string(date_value.c_str(),export_column->TFFC_layout,format.detail_lines_delineation.text_quote_mode_labeled.get(),delimiter);
/*040931*/                       } else
/*_______*/                          output_file->export_float(value,export_column->TFFC_layout,delimiter);
/*040930*/                    } else
/*041013*/                    {
...
/*041013*/                          output_file->export_string(period_description,export_column->TFFC_layout,format.detail_lines_delineation.text_quote_mode_labeled.get(),delimiter);
/*041013*/                       } else
/*040930*/                       output_file->export_string("N/A",export_column->TFFC_layout,format.detail_lines_delineation.text_quote_mode_labeled.get(),delimiter);
/*041013*/                    };
/*041013*/                 } else
/*040930*/                       output_file->export_string("N/A",export_column->TFFC_layout,format.detail_lines_delineation.text_quote_mode_labeled.get(),delimiter);
/*_______*/              };
...
/*040929*/     output_file->goto_row(format.detail_lines_delineation.start_row_number/*_1*/);
...
//_set_date_ranges___________________________________________________204-10-03_/
...
/*_______*/     if (format.column_headers_delineation.start_row_number/*050830 column_headers.start_row_1*/) output_column_headers();
   #endif
...
/*_______*/           // freeze pane
/*_______*/           excel_output_file->freeze_pane(1 , format.data_delineation?. start_row_number-1 );
#endif
...
//_start_data__________________________________________________________________/
...
,const char *delimiter)
{
   ...
   export_string(padded_value,column_format,Delineation::NO_QUOTE,delimiter);
};
//_export_integer______________________________________________________________/
...
)
{
   ...
   Ustring unpadded_value(CORN_float32_to_str(value, column_format.get_precision()));
   ...
   export_string(padded_value,column_format/*.start*/,Delineation::NO_QUOTE,delimiter);
};
//_export_float________________________________________________________________/
void UED_text_export_file::export_date
(const Date &date
,const ED_tabular_file_format::Column  &column_format
,const char *delimiter                                                           //050901
)
{
   ...
   export_string(date_text,column_format,Delineation::NO_QUOTE,delimiter);
}
//_export_date_________________________________________________________________/
...
{
   if (column_format.start_number > 0) // fixed width column
   {
      int str_len = str.length();
      size_t required_curr_line_length = (column_format.start_number /*070621 start*/  - 1 + str_len);
      if (current_line.length() < required_curr_line_length)
         current_line.resize(required_curr_line_length);
      current_line.replace(column_format.start_number /*070621 start*/  - 1,str_len,str);
   }
   else
   {
      current_line.append(delimiter);
      switch (quote_mode)
      {  case Delineation::NO_QUOTE : break;  // no quote
         case Delineation::SINGLE_QUOTE : current_line.append("'"); break;
         case Delineation::DOUBLE_QUOTE : current_line.append("\""); break;
...
//_export_string_______________________________________________________________/
#ifdef NN
...
#endif
//_write_string________________________________________________________________/
...
void UED_text_export_file::goto_row(int16 target_row)
{
   ...
   {
      int16 column_headers_start_row = format.column_headers_delineation.start_row_number;
      int16 detail_lines_start_row   = format.detail_lines_delineation.start_row_number;
...
{  BIFF234_File::write_integer(value,column_format.field_width);
}
//_export_integer______________________________________________________________/
...
,  const ED_tabular_file_format::Column  &column_format
,  const char *delimiter)
{  BIFF234_File::write_float(value,column_format.field_width,column_format.precision);
}
//_export_float________________________________________________________________/
...
{
   Ustring date_text(date.c_str());
   write_string_aligned(Spreadsheet_abstract::CSS_justify_left,date_text.c_str());
};
//______________________________________________________________________________
...
{
//   Spreadsheet_abstract::col = column_format.number - 1;
   BIFF234_File::write_string_aligned
      (Spreadsheet_abstract::CSS_justify_general,str.c_str());
//already advanced    Spreadsheet_abstract::col += 1; // <- auto advance cell     column_format.number - 1;
};
#endif
//______________________________________________________________________________
