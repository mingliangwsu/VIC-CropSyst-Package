...
**********************************************************************/
{
  ...
  //150929LML crop_data_struct     ***crop_state;                                            //150928LML
  //150929LML #endif
  ...
  //150929LML #if VIC_CROPSYST_VERSION>=3
  //150929LML crop_state = prcp->crop_state;                                                 //150928LML
  //150929LML #endif
  ...
  ********************************************/
  ...
  initialize_veg(veg_var[WET], veg_con, Nveg
               //150929LML #if VIC_CROPSYST_VERSION>=3
               //150929LML ,crop_state                                                       //150928LML
               //150929LML #endif
               #if VIC_CROPSYST_VERSION>=3
               ,soil_con
               //150930LML ,&prcp->grid_cell_crop_output
               #endif
               );
  if ( options.DIST_PRCP )
    initialize_veg(veg_var[DRY], veg_con, Nveg
               //150929LML #if VIC_CROPSYST_VERSION>=3
               //150929LML ,crop_state                                                       //150928LML
               //150929LML #endif
               #if VIC_CROPSYST_VERSION>=3
  ...
  ******************************************/
  ...
  for ( veg = 0 ; veg <= Nveg ; veg++) {
	...
	if ( soil_con->AreaFract[band] > 0. ) {
      ...
      ErrorFlag = distribute_node_moisture_properties(*soil_con, moist[veg][band], energy[veg][band]);
	  if ( ErrorFlag == ERROR ) return ( ErrorFlag );
	  ...
	  if(!options.QUICK_FLUX && soil_con->FS_ACTIVE) 
        find_0_degree_fronts(soil_con->Zsum_node, energy[veg][band].T, Nnodes,&energy[veg][band]);
	}
      }
    }
  }	
