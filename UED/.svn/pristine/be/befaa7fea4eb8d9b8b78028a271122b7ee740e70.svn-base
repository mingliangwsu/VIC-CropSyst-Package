...
{  float32 adjustments = 0;
   ...
   std::wstring name_with_var_code;
   adjustment_file_name.append_name_to(name_with_var_code);

   /* 181202 the name now only needs to contain the var code, not be the varcode completely
   ...
   */
   ...
   if      (name_with_var_code.find(L"tmax") != std::string::npos)  var_code = UED_STD_VC_Tmax;
   else if (name_with_var_code.find(L"tmin") != std::string::npos)  var_code = UED_STD_VC_Tmin;
   else if (name_with_var_code.find(L"srad") != std::string::npos)  var_code =
      #if (CS_VERSION >= 5)
      ...
      #endif
      ;
   else if (name_with_var_code.find(L"precip")!= std::string::npos) var_code = UED_STD_VC_precip;
   else if (name_with_var_code.find(L"rhmax") != std::string::npos) var_code = UED_STD_VC_RHmax;
   else if (name_with_var_code.find(L"rhmin") != std::string::npos) var_code = UED_STD_VC_RHmin;
   else if (name_with_var_code.find(L"dpmax") != std::string::npos) var_code = UED_STD_VC_Tdewmax;
   else if (name_with_var_code.find(L"dpmin") != std::string::npos) var_code = UED_STD_VC_Tdewmin;
   else if (name_with_var_code.find(L"ws")    != std::string::npos) var_code = UED_STD_VC_ws;
   else
   {  // raw var code is entered in hexidecimal
      var_code = CORN::wstr_to_nat32(name_with_var_code.c_str(),16);
   }


   /*181202
   now actually look for these as substrings of the filename
   ...
   {  // raw var code is entered in hexidecimal
      var_code = CORN::wstr_to_nat32(var_code_str.c_str(),16);
   }
*/
   }
...
std::clog << date << '\r';
      ...
      CORN::Date_clad_32 obs_date(date);
      interpolator->set_by_date(obs_date, adjustment);
      adjustments++;
   }
   return adjustments;
}
...
{  bool adjusted = CORN::OS::file_system_engine.copy_file(source_file,target_file,false);
       ...
       ;data_rec = target.goto_next_data_record())
   {
      ...
      Interpolator *interpolation = dynamic_cast<Interpolator *>(interpolations.find_nat32/*find*/(data_rec->get_variable_code()));
         // rename to interpolator 180426
      if (interpolation && interpolation->adjustment_mode > UNKNOWN_ADJUSTMENT)
      {
         ...
         CORN::Date_clad_32 earliest(data_rec->get_earliest_date_time(earliest_date_time_in_rec));
/*
if (earliest.get_date32() == 2010001)
std::clog << "reached" << std::endl;
*/
         CORN::Date_clad_32 latest  (data_rec->get_latest_date_time  (latest_date_time_in_rec));
         std::clog << "variable code:"  << std::hex << interpolation->var_code << "\t" << earliest.as_string() << "-" << latest.as_string() << std::endl;

         for (CORN::Date_clad_32 date(earliest)
             ;date <= latest
             ;date.inc_day())
         {
