...
{  FOR_EACH_IN(section,VV_Section,data_rec.sections,each_section)                //100110
   {  Fielded_sheet *fielded_sheet = get_fielded_sheet(section->get_label().c_str());
      Workbook::Sheet *sheet = fielded_sheet->get_sheet();
            //for each entry in current section:
      FOR_EACH_IN(entry,VV_abstract_entry,section->entries,each_entry)           //100110
      {  // lookup field in actual fields:
         std::string sheet_name(entry->get_tabular_column()->sheet.name);        //200429_111213
         ...
         //200429 nat8 curr_col = entry->get_tabular_column()->position-1;       //190506_111213
         nat8 curr_col = entry->get_tabular_column()->delineation.position-1;    //200429_190506_111213
         {  std::string get_buf;
            switch (entry->get_type())
