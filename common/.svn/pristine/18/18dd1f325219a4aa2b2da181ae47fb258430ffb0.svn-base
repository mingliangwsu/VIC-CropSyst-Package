...
, public extends_ CS::Evaporator_intervals                                       //160717_160523
{
 protected:
   ...
   const Soil::Layers_interface  &soil_layers;                                   //060330
...
090302 RLN Dont delete. This should be correct
 ...
 public:
   float64 calc_thermal_conductance
      (bool     snow_or_residue_cover)                             calculation_;
 private: // thermal conductance locals methods
   float64 calc_thermal_conductivity_conduction
      (float64  bulk_density)                                      calculation_;
   float64 calc_convective_heat_transfer_coef
      ( float64 bulk_density
      , float64 depth
      , bool    snow_or_residue_cover)                             calculation_;
   float64 calc_wind_speed_within_residue
      ( float64 bulk_density
      , float64 depth)                                             calculation_;
   float64 calc_wind_attenuation_coef
      (float64 bulk_density)                                       calculation_;
   float64 calc_surface_windspeed( float64 depth)                  calculation_;
 public: // inspection
   RENDER_INSPECTORS_DECLARATION;                                                //160627
};
//_Residues_common_____________________________________________________________/
#endif
