...
double SnowPackEnergyBalance(double TSurf, va_list ap)
{
  ...
  /* Apply the stability correction to the aerodynamic resistance 
     ...
     reference level */
  if (Wind > 0.0) Ra_used[0] = Ra / StabilityCorrection(Z, 0.f, TMean, Tair, Wind, Z0[2]);
  else            Ra_used[0] = param.HUGE_RESIST;
  ...
  *SensibleHeat = AirDens * Cp * (Tair - TMean) / Ra_used[0];
  ...
  if ( SnowCoverFract > 0 ) {
    *(AdvectedSensibleHeat) = calc_advected_sensible_heat(SnowCoverFract,
						     AirDens, Tair, TGrnd, 
						     Ra_used[0]);
  }
  ...
  *surface_flux = SurfaceMassFlux * Dt / Density;
  ...
  if ( TMean == 0 ) *AdvectedEnergy = (CH_WATER * (Tair) * Rain) / (Dt);
  else              *AdvectedEnergy = 0.;
  ...
  *RefreezeEnergy = (SurfaceLiquidWater * Lf * Density)/(Dt);
  if (TSurf == 0.0 && RestTerm > -(*RefreezeEnergy)) {
    *RefreezeEnergy = -RestTerm;  /* available energy input over cold content
                                    ...
                                    (energy out of pack)*/ 
    RestTerm = 0.0;
    //fprintf(stderr,"LongSnowIn %f TMean %f \n",LongSnowIn,TMean);
  } else {
