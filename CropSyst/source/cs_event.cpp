...
//_log_automatic_irr_event_____________________________________________________/
#endif
//______________________________________________________________________________
      ...
      {  //bool is_perennial = false;                                            //021217
         ...
         else
         {
            ...
            #endif
            {
               ...
               std::clog << "sowing:" << sowing_op.plant_by_date << "\t" << sowing_op.crop_filename << std::endl;
         ...
         if (                                                                    //021217
               ...
               #endif
               (sowing_op.management_filename.length()>1 &&
                  CORN::OS::file_system_engine.exists(sowing_op.management_filename))) //000313
         {
            ...
            scenario_event_list.synchronize
               (Synchronization(BEFORE_PLANTING_MODE)   ,planting_date);         //130827_030611
...
//_schedule_planting_event__________________________________________2005-01-03_/
...
//_resolve_and_schedule_____________________________________________2016-05-12_/
bool Land_unit_simulation_VX::load_rotation_planting_events()
{
   #ifdef VERBOSE
   if (CORN::global_arguments->is_verbose(1))                                    //191015
   {
   ...
   {  CropSyst::Sowing_operation *sowing_op = entry->sowing_op;
      ...
      {  // This is just a hack to allow CropSyst proper to use OFoot scenarios (with relative date).
         ...
         {  CORN::Year relative_sow_year = entry->begin_sync.get_year();
            ...
            {  Sowing_event *planting_event = new Sowing_event(*entry);
               ...
               CORN::Date_clad_32 actual_plant_date
                  (actual_plant_year,entry->begin_sync.get_DOY());
               Sowing_operation *actual_op = planting_event->sowing_op;
               resolve_and_schedule(actual_plant_date,*actual_op
                  ,entry->ID.c_str(),rotation_dir);
               planting_event->begin_sync.sync_mode_clad.set(ACTUAL_DATE_MODE);
      ...
      {
      entry->synchronize(Synchronization(SIMULATION_BEGIN_DATE,0)
         ,scenario_control_and_model_options.start_date ASSOCIATE_THIS);         //030609
      entry->synchronize(Synchronization(ACTUAL_DATE_MODE)
         ,scenario_control_and_model_options.start_date ASSOCIATE_THIS);         //030609
...
//_load_rotation_planting_events_______________________________________________/
...
//_profile_print_event::profile_print_event____________________________________/
...
//_profile_print_event::label_string________________________________2017-04-23_/
bool profile_print_event::Print_operation::process()
{  record_soil_profile_today = true;
   return true;
}
//_profile_print_event::Print_operation::process____________________2001-02-02_/
#endif
