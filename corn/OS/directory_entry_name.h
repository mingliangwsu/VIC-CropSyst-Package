...
{public:
   enum Qualification                                                            //121102
      ...
      relative partial (01) - means that when the composed DEN is
         ...
         - DENs in subdirectories will have partial paths.
   ...
   static const Path_inclusion include_qualification              ;
   static const Path_inclusion include_qualification_designated   ;
 ...
 public: // queries
   ...
   virtual bool has_in_path_wstr
      ...
      ,Path_inclusion inclusion = include_qualification_designated
...
/*190114 These should be function of file_system engine
 (this class should not be dependent on file system engine)
...
*/
   ...
   /*190114 This was broken, now using compose_relative_to
   virtual std::wstring *render_relative_to
      (const Directory_name &qualified_reference_directory_name
      ,Qualification qualification = relative_strict_qualification)rendition_=0; //151221
   */
   virtual bool compose_relative_to
      (const Directory_name &qualified_reference_directory_name
      ,std::wstring &composed
      ,Qualification qualification = relative_strict_qualification)     const=0; //151221
      /**<
   ...
   case
      ...
      A:\X\Y\Z
      renders:
      d:\A\B\C\f.ext  (unchanged because different file systems)
   case
      A\B\C\D\E\f.ext
      ref dir
      B\C\D
      renders
      A\B\C\D\E\f.ext  failure, the paths don't start out common
   case
      ..\..\A\B\C\f.exe
      ref dir
      X\Y
   ...
   case
      ...
      ..\..
      renders
      A\B\C\f.exe  with failure (ref dir cannot be relative);

   case this is unqualified
      this:
      f.exe
      ref dir:
      d:\A\B\C\
      renders:
      f.exe failure no common path
      or
      .\f.ext  if

   Note that there could be a version of this class in file_system
   which uses the current working directory, for unqualified names.
      **/
   ...
   virtual CORN::Container & append_path_list_to // was get_path_list
      (CORN::Container &buffer                                                   //181030
      ,Path_inclusion inclusion = include_qualification_designated)     const=0;
...
public:  // Queries:
   ...
   virtual bool is_relative()                                      affirmation_;
      ...
      **/
   inline virtual bool is_absolute()      affirmation_ { return !is_relative();} //190114
      /**< \return true if the name has a relative path
         Note that an unqualified name is also considered to be relative.
         Use has_relative_path() if you do not want to consider unqualifed names.
      **/

   virtual bool is_qualified_relative()                          affirmation_=0;
      /**< \return true if there is a path and it is relative.
      **/
   #define has_relative_path is_qualified_relative
...
//_Directory_entry_name_____________________________________________2009-12-19_/
