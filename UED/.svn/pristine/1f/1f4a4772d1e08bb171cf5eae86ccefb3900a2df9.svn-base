...
,{'R',CORN::other_real_quality          }  //  - Precipitation not reported; estimated as "O"
...
private:
   ...
   void import_record(NCDC_element_record &element_record);
...
{
   comma_delimited = header_line.find(",") != CORN_npos;                  // 070424 these may need to be find_substring
   station_name_present = header_line.find("STATION") != CORN_npos;
   data_flags_present = (header_line.find(",F,") != CORN_npos)
                     || (header_line.find(" F ") != CORN_npos);
};
//______________________________________________________________________________
...
cout << '\r' << (int)year << ' ' << (int)month << ' ' <<element_record.meteorological_element_type_labeled.get_label();
      ...
      for (DOM dom = 1; dom <=  CORN::date_get_days_in_month( year, month); dom++)
      {
         CORN::Date date_ymd;
         date_ymd.set_YMD((Year)year,(Month)month,(DOM)dom);
         CORN::Datetime64 set_date = date_ymd.get_datetime64(); //
...
uint32 NCDC_daily_surface_data_format_convertor::perform_import()
{
   ...
   std::string buffer;
   std::getline(input_file,buffer);    // The header line
   identify_format_options(buffer);
   std::getline(input_file,buffer);    // The dash line
/*100203
   CORN::Ustring buffer;
