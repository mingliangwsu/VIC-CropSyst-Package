...
void FMT_to_TDF_Convertor::Report_tabular_format::convert(Format_file_type format_file_type,const char *tdf_filename)
{
   ...
   header_terminator = 0;
   definer="CropSyst fmt_to_tdf convertor (contact rnelson@mail.wsu.edu)";
   // inception = already set as default today
#ifdef NYI
...
/*______*/     quotation = "\""; //200220 text__quote_mode_labeled.set(DOUBLE_QUOTE);
...
//   UED_record_type_code = xxxx;
   ...
   {  // Daily report static
      ...
      report_label->col_num = 1;
      report_label->row_num = 1;
      ...
      metadata_fields_numbered.append(report_label);                               //101017
   ...
   {  // scenario name
      ...
      scenario_name->col_num = 1;
      scenario_name->row_num = 2;
      ...
      //101017 header_fields_1based
      metadata_fields_numbered.append(scenario_name);                              //101017
   }
   if (format_file_type == FILE_TYPE_Excel)   //050830 Currently the text format does not have headers
   {  // soil name
      ...
      //101017 header_fields_1based
      metadata_fields_numbered.append(soil_name);                                  //101017
   }
   if (format_file_type == FILE_TYPE_Excel)   //050830 Currently the text format does not have headers
   {  // location name
   ...
   {  // version label static
      ...
      //101017 header_fields_1based
      metadata_fields_numbered.append(version_label);                            //101017
   }
   if (format_file_type == FILE_TYPE_Excel)
   {  // version number
      Metadata_field *version_number = new Metadata_field;                       //101017
      version_number->description = "version number";
      version_number->col_num = 4;
      version_number->row_num = 1;
      ...
      metadata_fields_numbered.append(version_number);                           //101017
   }
#ifdef NYI
   ...
   {  // build date label static
      ...
      build_date_label->col_num = 1;
      build_date_label->row_num = 1;
   ...
   {  case FILE_TYPE_Excel :
      {  // column headers deliniation
         column_headers_delineation.delimit_mode_labeled.set(Delineation::intrinsic_TABULATION);
         //200220 column_headers_delineation.text__quote_mode_labeled.set(Delineation::DOUBLE_QUOTE);
         column_headers_delineation.separation.quotation = "\"";                 //200220
         column_headers_delineation.start_row_number =  6 ;
         ...
         detail_lines_delineation.delimit_mode_labeled.set(Delineation::intrinsic_TABULATION);
         detail_lines_delineation.start_row_number= 12 ;
         detail_lines_delineation.separation.quotation = "\"";                   //200220
         //200220 detail_lines_delineation.text__quote_mode_labeled.set(Delineation::DOUBLE_QUOTE);
         // delimiters don't apply to Excel format
      } break;
     case FILE_TYPE_text:
      {  // column headers deliniation
         column_headers_delineation.tabulation_labeled.set(Delineation::delimit_TABULATION);
         column_headers_delineation.start_row_number   = 1;

         column_headers_delineation.separation.quotation = "\"";                 //200220
         column_headers_delineation.separation.continuous= false;                //200220
         column_headers_delineation.separation.delimiter = " ";                  //200220
         column_headers_delineation.separation.decimal   = ".";                  //200220
         ...
         */
         ...
         column_headers_delineation.units_rows           = 1;
         // detail line delineation
         detail_lines_delineation.delimit_mode_labeled.set(Delineation::delimit_TABULATION);
         detail_lines_delineation.start_row_number=3;

         detail_lines_delineation.separation.quotation = "\"";                   //200220
         detail_lines_delineation.separation.continuous= false;                  //200220
         detail_lines_delineation.separation.delimiter = ',';                    //200220
         detail_lines_delineation.separation.decimal= ".";                       //200220


         /*200220
         detail_lines_delineation.text__quote_mode_labeled.set(Delineation::DOUBLE_QUOTE);
   ...
   {  // sel_var_code is 1 based.
      ...
      column->variable_code_clad = variable_code;

      CORN::Data_type data_type = CORN::DT_float32;
      ...
      if (variable_code == CSVC_crop_base_planting_date_YMD)      {data_type = CORN::DT_date;column->variable_code_clad =CSVC_crop_base_planting_date_YD;      };
      if (variable_code == CSVC_crop_base_planting_date_YMD)      {data_type = CORN::DT_date;column->variable_code_clad =CSVC_crop_base_planting_date_YD;      };
      if (variable_code == CSVC_crop_base_emergence_date_YMD)     {data_type = CORN::DT_date;column->variable_code_clad =CSVC_crop_base_emergence_date_YD;     };
      if (variable_code == CSVC_crop_base_flowering_date_YMD)     {data_type = CORN::DT_date;column->variable_code_clad =CSVC_crop_base_flowering_date_YD;     };
      if (variable_code == CSVC_crop_base_grain_filling_date_YMD) {data_type = CORN::DT_date;column->variable_code_clad =CSVC_crop_base_grain_filling_date_YD; };
      if (variable_code == CSVC_crop_base_peak_LAI_date_YMD)      {data_type = CORN::DT_date;column->variable_code_clad =CSVC_crop_base_peak_LAI_date_YD;      };
      if (variable_code == CSVC_crop_base_maturity_date_YMD)      {data_type = CORN::DT_date;column->variable_code_clad =CSVC_crop_base_maturity_date_YD;      };
      if (variable_code == CSVC_crop_base_harvest_date_YMD)       {data_type = CORN::DT_date;column->variable_code_clad =CSVC_crop_base_harvest_date_YD;       };
      ...
      // (This is not in if block)
      {
            ...
            {  // column header and rows are the same
               if (report_var_opts[rvo].ID == variable_code)
              {
                  ...
                  if (column->period >= 0)
                     column->data_type_labeled.set(CORN::DT_string);
                  else
                     column->data_type_labeled.set(data_type);
#ifdef NYI
/*______*/        {DT_char
/*______*/        ,DT_string
/*______*/        ,DT_integer
...
#endif
            switch (format_file_type)
            {  case FILE_TYPE_Excel :  column->start_number = i; break;
               case FILE_TYPE_text  :
                     if (detail_lines_delineation.delimit_mode_labeled.get() == Delineation::fixed_TABULATION)
                     {
                        column->start_number           = text_column_start;
                        text_column_start += report_var_opts[rvo].w;  //append the width of this column to the next so we don't override
                     } else
                        column->start_number           = 0;
                  break;
            };
            column->field_width= report_var_opts[rvo].w; // Doesn't really apply to spreadsheets, but we may use eventually it for cell formatting
            column->precision  = report_var_opts[rvo].d;
...
//            if (format_file_type== FILE_TYPE_text)
            ...
            column->record_type_clad = record_type_code_used_by_all_records; //.set(record_type_code_used_by_all_records.get());
            ...
            nat8 var_qualifier = column->variable_code_clad.get_qualifier();
            if (var_qualifier)
            {
               strcat(description,",\"");
               char *qual = "";
   ...
   {  // Columns
      if (column->caption.length() == 0)
      {
         ...
         ltoa(column->variable_code_clad.get(),label_buffer,16);
...
/*______*/     delimit_mode_labeled.set(delimit_TABULATION);
/*______*/
/*______*/     continuous_delimiter = false;
/*______*/     delimiter_space      = false;
/*______*/     delimiter_comma      = true;
...
//NYI /*050517_*/    notes = "";
   ...
   column_headers_delineation.separation.quotation = "\"";                       //200220
   //200220 column_headers_delineation.text__quote_mode_labeled.set(Delineation::DOUBLE_QUOTE);
   column_headers_delineation.start_row_number   = 12;
   // delimiters don't apply to Excel format
   column_headers_delineation.rows        = 1;
   column_headers_delineation.units_row_offset   = 1; // offset from column header start row
   column_headers_delineation.units_rows  = 1;
   ...
   // detail line delineation
   detail_lines_delineation.delimit_mode_labeled.set(Delineation::intrinsic_TABULATION);
   detail_lines_delineation.start_row_number=14;
   detail_lines_delineation.separation.quotation = "\"";                         //200220
   //200220 detail_lines_delineation.text__quote_mode_labeled.set(Delineation::DOUBLE_QUOTE);
   // delimiters don't apply to Excel format
...
//   UED_record_type_code = xxxx;
   ...
   {  // Daily report static
      ...
      metadata_fields_numbered        //101017
         .append(report_label);
   }
...
*/
   ...
   {  // date
      Column *column = new Column;
      column->variable_code_clad = CSVC_weather_date_year_day;
      ...
      */
      ...
      column->record_type_clad = Record_type_distinguisher_interface::UED_data_set_year_var_units; //.set(record_type_code_used_by_all_records.get());
      column->units_caption = "YYYY/MM/DD(DDD)";
      column->statistic_labeled.set(value_statistic);
   ...
   {  // Year
      ...
      column->caption               = "Date";
      column->record_date           = true;
      column->variable_code_clad   = CSVC_weather_date_year;
      column->data_type_labeled.set(CORN::/*Tabular_column::*/DT_int16);
      column->start_number          = FILE_TYPE_Excel ? 1 : 1 ;   // Warning this is only for delimited mode need to increment text_column_start with a width
   ...
   {  // Month
      ...
      column->caption               = "Date";
      column->record_date           = true;
      column->variable_code_clad   = CSVC_weather_date_month;
...
//      column->statistic_labeled.set(value_statistic);
   ...
   {  // DOM
      ...
      column->caption               = "Date";
      column->record_date           = true;
      column->variable_code_clad   = CSVC_weather_date_DOM;
      column->data_type_labeled.set(CORN::DT_int16);
      column->start_number          = FILE_TYPE_Excel ? 1 : 1 ;   // Warning this is only for delimited mode need to increment text_column_start with a width
      column->field_width           = 4; // Doesn't really apply to spreadsheets, but we may use eventually it for cell formatting
      column->precision             = 0;
   ...
   {  // DOY
      ...
      column->caption               = "Date";
      column->record_date           = true;
      column->variable_code_clad   = CSVC_weather_date_DOY;
      column->data_type_labeled.set(CORN::DT_int16);
      column->start_number          = FILE_TYPE_Excel ? 1 : 1 ;   // Warning this is only for delimited mode need to increment text_column_start with a width
      column->field_width           = 4; // Doesn't really apply to spreadsheets, but we may use eventually it for cell formatting
      column->precision             = 0;
   ...
   {  // root depth column
      ...
      column->variable_code_clad   = CSVC_crop_base_root_depth;
      column->data_type_labeled.set(CORN::DT_float32);
      column->start_number          = 5;
      column->field_width           = 8; // Doesn't really apply to spreadsheets, but we may use eventually it for cell formatting
      column->precision             = 5;
      /*200224
      column->spread_over_columns   = false; // does not apply to CropSyst reports
   ...
   for (int i = 1; i <= MAX_soil_sublayers; i++)
   {
      ...
      column->variable_code_clad = smart_var_code;
      column->variable_code_clad.(i);
            ...
            */
            ...
            column->record_type_clad = Record_type_distinguisher_interface::UED_data_set_year_var_units;
            column->statistic_labeled.set(value_statistic);
      columns_1based.append(column);
   };
   FOR_EACH_IN(column,Column,columns_1based,each_column)
