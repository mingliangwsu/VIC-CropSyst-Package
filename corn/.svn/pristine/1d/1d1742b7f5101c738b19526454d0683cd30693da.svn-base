...
//_get_english_length_to_yards_multiplier______________________________________/
bool Units_convertor::normalize_customary_units_to_metric(float32 &from_value, Units_component &from_units) const

// Should rename to normalize_CU_to_SI

{  bool normalized = false;                                                      //190717
...
//                   case SUE_gallon       :  amount_to_gallons = 1.0;            break;
               ...
               case UM_volume :                                                  //190422
               {
                  float64 amount_to_yard3 = 1.0;
                  nat8 submeasure = from_units.get_submeasure_index();
                  switch (submeasure)
                  {  case UE_foot3 : amount_to_yard3 = 0.037037037037037; break;
                     //NYI case inch3 : amount_to_yard3 = 2.14334705075446E-05; break;
                     //case yard3 : amount_to_yard3 = 1.0; break;
                  } // switch
                  if (from_units.is_denominator())
                     amount_to_yard3 = (1.0 / amount_to_yard3);
                  float64 from_value_yard3 = from_value * amount_to_yard3;
                  from_value = (float32)yard3_to_m3(from_value_yard3);
                  from_units = UC_cubic_meter_volume;
                  normalized = true;                                             //190717
               } break;
               /// Warning I need to implement other units
               // As with normalization length to yards, we normalize weights to grains
               case UM_pressure_atmospheric :
               {  // normalize to atmospheres

                  float64 amount_to_atms = 1.0;
                  switch (from_units.subcode) // ???
                  {  case UE_atmospheric_pressure_inch_Hg :
                        amount_to_atms = 1.0 / 29.921;
                     break;
                     /*NYI
                     case UE_atmospheric_pressure_psi:
                        amount_to_atms = 1.0 / 14.6959;
                     break;
                     */
                  }
                  if (from_units.is_denominator())
                     amount_to_atms = (1.0 / amount_to_atms);
                  float64 from_value_atm = from_value * amount_to_atms;
                  from_value =  (float32)atm_to_pascal(from_value_atm);
                  from_units = UC_pascal_pressure;
                  normalized = true;                                             //190717
               } break;
            }
         break;
...
//_normalize_customary_units_to_metric_________________________________________/
...
//_convert_subunit/_____________________________________________________________
...
{  bool result = true;
   ...
   result &= normalize_to_metric_base_units(from_value, from_SUC);
...
//_convert_metric_to_customary______________________________________2005-06-13_/
bool Units_convertor::normalize_to_metric_base_units(float32 &from_value,Units_component &from_SUC) const
{
...
//_normalize_to_metric_base_units___________________________________2005-06-13_/
...
//_convert_meter_to_customary_______________________________________2005-06-13_/
...
//_convert__________________________________________________________2010-03-03_/
bool Units_convertor::convert
(float32 from_value
,Units_code from_units  // may be modified but only locally
,float64 &to_value
...
//040715 now we just have volume       case UC_cubic_meter_liquid_volume :
      ...
      break;
      case UC_tn_per_ha :                                                        //200808
         if (from_units == UC_kg_m2) // Denominator is not the same measure      //200808
         {  to_value =  (float32)kg_m2_to_tonne_ha(from_value);                  //200808
            converted = true;                                                    //200808
         }
      break;
      case UC_tn_per_m2 :                                                        //200808
         if (from_units == UC_kg_m2) // Denominator is not the same measure      //200808
         {  to_value =  (float32)kg_to_tonne(from_value);                        //200808
            converted = true;                                                    //200808
         }
      break;
...
//_convert_time_____________________________________________________2005-04-21_/
...
//_convert_with_time_step___________________________________________2005-04-21_/
...
//_get_seconds_in___________________________________________________2005-04-21_/
