#include "cell_reader_Excel_XLS.h"
...
public:
   Cell_shared_string(nat16  _row,nat16 _col,const std::wstring &_text)
      : row(_row)
      , col(_col)
      , text(_text)
      {}
   inline virtual nat32  get_row()                                         const { return row; }
   virtual nat32  get_col()                                                const {return col;}
   virtual const std::string  & set_string  (const std::string &value)  modification_ { return value;} // reader has no modification capabilities
   virtual const std::wstring & set_wstring (const std::wstring &value) modification_ { return value;} // reader has no modification capabilities
   virtual std::string  & get_string  (std::string &value)           const  { wstring_to_string(text,value);return value;}
   virtual std::wstring & get_wstring (std::wstring &value)          const  { value.assign(text);           return value;}
};
//______________________________________________________________________________
Cell_reader_Excel_XLS::Cell_reader_Excel_XLS
(const std::string &file_name)
//190506 (const CORN::OS::File_name &file_name)
:  Compound_file_binary_format(file_name.c_str())
...
{}
//______________________________________________________________________________
nat32 Cell_reader_Excel_XLS::read(Cell_receiver &receiver)
{  nat32 records_encountered = 0;
   Binary_file_interface  &binary_file = provide_user_stream(L"Workbook");       //110828
   ...
   delete BOF_record;
   for  (bool _at_eof = !binary_file.good() //110828 binary_file == NULL
        ;!_at_eof
...
std::cout << "reached"<< "\n"; //100826 std::endl;  borland linker complians
      ...
      {
         BIFF_record *record = render_BIFF_record(record_code,record_length,format); // (BIFF_record *)instanciate_record(record_code);
...
//______________________________________________________________________________
bool Cell_reader_Excel_XLS::read_and_process_record
(Cell_receiver &receiver
,BIFF_record *record)     // record needs to be a pointer because we do dynamic casting and dont want to throw exceptions
{  bool processed = true;
   record->read_body( provide_user_stream(L"Workbook") /*110828  binary_file*/);
      ...
      {     processed = true;
            switch (record->get_code())
            {
               ...
               case BIFF8_CODE_LABELSST :
               {
                  ...
                  if (SST_label)
                  {
                  const BIFF8_rec_SST::Item *SST_string = (const BIFF8_rec_SST::Item *)shared_string_table->get_at(SST_label->get_shared_string_table_index());
                  Cell_shared_string *shared_string_cell = new Cell_shared_string(SST_label->get_row(),SST_label->get_col(),*SST_string);
               ...
               case BIFF2_CODE_STRING :
               {
                  ...
                  if (string_rec)
                  {
                     BIFF2_rec_LABEL *label_rec= new BIFF2_rec_LABEL(last_formula_string_row,last_formula_string_col,0,0,0,string_rec->text);
...
//______________________________________________________________________________
Binary_record *Cell_reader_Excel_XLS::instanciate_record(Record_code code) const
{
   // Note that this is only to implement instanciate_record,
   // we dont actually use instanciate_record in this program.
