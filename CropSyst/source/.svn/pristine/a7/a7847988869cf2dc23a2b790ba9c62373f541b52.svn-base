...
Interception_architectural_Kemanian_Stockle::Interception_architectural_Kemanian_Stockle
...
{}
//_Orchard_interception_Kemanian_Stockle:constructor___________________________/
float64 Interception_architectural_Kemanian_Stockle::calc_sine_solar_elevation
(float64 sine_solar_declination
,Hour hour)                                                         calculation_
{  //Dim Radian As Single, Asin As Single, SSE1 As Single, SSE2 As Single
   ...
   return CORN::must_be_between<float64>(SSE1,-0.9999,0.9999);
}
//_calc_sine_solar_elevation___________________________________________________/
float64 Interception_architectural_Kemanian_Stockle::calc_cosine_solar_azimuth
(float64 sine_solar_declination
,float64 sin_solar_elev)                                            calculation_
{  float64 asin_SSE = asin(sin_solar_elev);
   ...
   return CORN::must_be_between<float64>(CSA1,-0.9999,0.9999);
}
//_calc_cosine_solar_azimuth___________________________________________________/
float64 Interception_architectural_Kemanian_Stockle::calc_shadow_angle
(float64 solar_elev_radians
,float64 solar_azimuth_radians)                                     calculation_
{
   float64 Sine_Row_Solar_Az_Angle
      = fabs(sin(solar_azimuth_radians - row_azimuth_radians));                  //020410
   float64 _SA = (Sine_Row_Solar_Az_Angle == 0)
   ? 1.570796
   : atan(tan(solar_elev_radians) / Sine_Row_Solar_Az_Angle);
   return _SA;
}
//_calc_shadow_angle___________________________________________________________/
float64 Interception_architectural_Kemanian_Stockle::calc_shadow_length
(float64 shadow_angle_radians)                                      calculation_
{  float64 row_height = tree_canopy->height.get();                               //071117
   float64 row_width  = tree_canopy->width.get();                                //071117
   float64 shadow_length
      = (row_height - row_width + row_width / cos(shadow_angle_radians))
        / tan(shadow_angle_radians);
   return shadow_length;
}
//_calc_shadow_length__________________________________________________________/
float64 Interception_architectural_Kemanian_Stockle::RSRD
(float64 sin_solar_elev
,float64  SA)                                                       calculation_
// SA units? radians or sine?
...
{  float64 row_height = tree_canopy->height.get();                               //071117
   ...
   : fabs((row_height - row_width + row_width / cos(SA))
      / (tan(SA) * layout.spacing.row));                                         //020410
   return CORN::must_be_between<float64>(ss,0.0,1.0);
}
//_RSRD________________________________________________________________________/
 float64 Interception_architectural_Kemanian_Stockle::calc_hourly_incident_angle
 (float64 solar_elev_radians
 ,float64 solar_azimuth_radians )                                   calculation_
{
 //This function determines the cosine of the angle of incidence, defined as the
 //angle measured from a normal to the surface with given aspect and slope.
 // The equivalent solar elevation angle for the inclined plane is obtained
 // by substracting the incident angle from 90 degrees or pi radians.
...
//_calc_hourly_incident_angle__________________________________________________/
float64 Interception_architectural_Kemanian_Stockle::calc_corrected_shadow_length
(float64 Shadow_Length
,float64 solar_elev_radians
,float64 solar_azimuth_radians)                                     calculation_
{
 // This function corrects shadow length for the effect of aspect and slope
   float64 Cos_Solar_Aspect_Az_Angle
      = cos(slope_aspect_from_south_radians - solar_azimuth_radians);
   // Determines slope component in the direction of the shadow
   float64 Slope_Comp = slope_radians * Cos_Solar_Aspect_Az_Angle;
   float64 Cor_Factor
      = sin(solar_elev_radians)/sin(3.14159 - solar_elev_radians - Slope_Comp);
   float64 _CorSL = Shadow_Length * Cor_Factor;
   float64 row_width  = tree_canopy->width.get();                                //071117
   if ((_CorSL < row_width) && (_CorSL > 0) )
       _CorSL = row_width;
   return  CORN::must_be_0_or_greater<float64>(_CorSL);
}
//_calc_corrected_shadow_length________________________________________________/
// Warning not sure why this function was originally declared variant.
float64 Interception_architectural_Kemanian_Stockle
::calc_hourly_direct_radiation_potential(float64 sin_solar_elev)    calculation_
{  float64 _POTRAD = 1360.0 * sin_solar_elev * pow(0.75,(1.0 / sin_solar_elev));
   return CORN::must_be_0_or_greater<float64>(_POTRAD);
}
//_calc_hourly_direct_radiation_potential______________________________________/
void Interception_architectural_Kemanian_Stockle::get_hourly_interception
(Hour        hour
...
{  Hourly_Direct_Row_Int = 0.0;
   ...
   float64 solar_elev_radians = CORN::must_be_0_or_greater<float64>(asin(Sine_Solar_Elev));
     //this is arcsin: Atn(Sine_Solar_Elev / Sqr(-Sine_Solar_Elev * Sine_Solar_Elev + 1))
   if (Sine_Solar_Elev > 0)
   {
      float64 Cosine_Solar_Azimuth = calc_cosine_solar_azimuth
         (Sine_Solar_Dec, Sine_Solar_Elev);
      float64 solar_azimuth_radians = (hour <= 11)
         ? acos(Cosine_Solar_Azimuth)
         : (1.5 * 3.14159 + asin(Cosine_Solar_Azimuth));
      float64 Hourly_Incident_Angle = calc_hourly_incident_angle
         (solar_elev_radians, solar_azimuth_radians);
      ...
      {  float64 Shadow_Angle = calc_shadow_angle
            (solar_elev_radians, solar_azimuth_radians);
         Shadow_Length = calc_shadow_length(Shadow_Angle);
         Corrected_Shadow_Length = calc_corrected_shadow_length
               (Shadow_Length,solar_elev_radians,solar_azimuth_radians);
      }
      ...
      float64 row_distance = layout.spacing.row;                                 //020410
      Hourly_Direct_Row_Int = CORN::must_be_between<float64> // CORN_must_be_between
         (Corrected_Shadow_Length / row_distance,0.0,1.0);
      Hourly_Diffuse_Row_Int = CORN::must_be_between<float64> // CORN_must_be_between
         (Shadow_Length / row_distance,0.0,1.0);
...
//_get_hourly_interception_____________________________________________________/
void  Interception_architectural_Kemanian_Stockle::get_daily_interception
...
//_get_daily_interception______________________________________________________/
float64 Interception_architectural_Kemanian_Stockle
...
{  solid_canopy_interception = 1.0;                                              //000717
   ...
   float64 diffuse_rad_fract = CORN::must_be_between<float64>
      ((0.3+0.7 * fract_cloud_cover),0.0,1.0);
