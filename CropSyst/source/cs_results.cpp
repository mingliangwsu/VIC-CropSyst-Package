...
//_PAW_bar__________________________________________________________1997-12-31_/
#if (CROPSYST_VERSION == 4)
#ifdef PRINT_PROFILE
...
//_setup_profile_print_events___________________________2005-12-01__2000-05-26_/
#endif
#endif
...
,float64 root_depth)
{
   ...
   if (scenario_control_and_model_options.run_nitrogen)                          //130827
     ...
     profile << ' ' << (TL_TOTALHEAD2) << std::endl
             << " # #   m    m   m3/m3  %                                                        " << endl;
   const CropSyst::Soil_components *soil = ref_soil();                           //060803
   if (soil)                                                                     //060803
   {  const Soil::Layers_interface *layers = soil->ref_layers();
      for (nat8 sublayer=1;sublayer<=std::min<nat8>(20,layers->count());sublayer++)
      {
         profile.width(2);
         ...
         CropSyst::Crop_model_interface *crop_active =                           //131008NS
         #ifdef INTERCROPPING
            get_predominant_crop();                                              //130716
         #else
            crop_active_or_intercrop;
         #endif
         if (!crop_active || root_depth <= 0.000001)                             //181107
            //181107 crop_active->ref_phenology().get_growth_stage_sequence() == NGS_GERMINATION)
               depth_marker = ' ';
         ...
         if (scenario_control_and_model_options.run_nitrogen && soil->chemicals) //130827
         {
            ...
            profile <<  setw(7) << setprecision(3)<<
               ...
               #ifdef OLD_N_XFER
               molecular_to_elemental*(soil->chemicals->NO3->transformed_from_M[sublayer])
               #else
...
//NYI              profile <<  setw(7) << setprecision(3)<< ((soil->chemicals->denitrification_N_E_output[sublayer])*over_m2_to_ha) << ' ' ;
      ...
      float64 ultimate_freezing_depth =  soil_freezing_optional
         ? soil_freezing_optional->get_ultimate_freezing_depth() : 0.0;          //070824
   ...
   if (scenario_control_and_model_options.run_nitrogen && soil->chemicals)       //130827
   {
      ...
      profile << (float)soil->chemicals->NO3->molecular_to_elemental
         *(soil->chemicals->NO3->get_current_profile_content_M()) *over_m2_to_ha<< ' '; //981028
      profile.width(7); profile.precision(3);
      profile << (float)soil->chemicals->NH4->molecular_to_elemental
         *(soil->chemicals->NH4->get_current_profile_content_M()) *over_m2_to_ha << ' ';   //981028
      profile.width(7); profile.precision(3);    /*ACC_mineralization*/

      float64 X_transformed = soil->chemicals->NH4->ANNL_bal_accums_M
      ? soil->chemicals->NH4->molecular_to_elemental
         *(soil->chemicals->NH4->ANNL_bal_accums_M->transformed_from)*over_m2_to_ha
      : 0.0;
      profile << X_transformed << ' ';

      float64 Y_transformed = soil->chemicals->NO3->ANNL_bal_accums_M
      ? soil->chemicals->NO3->molecular_to_elemental
         *(soil->chemicals->NO3->ANNL_bal_accums_M->transformed_from) *over_m2_to_ha
      : 0.0;
      ...
      float64 Z_transformed =  soil->chemicals->NO3->ANNL_bal_accums_M
      ? soil->chemicals->NO3->molecular_to_elemental
         *(soil->chemicals->NO3->ANNL_bal_accums_M->transformed_to) *over_m2_to_ha
      : 0.0;
...
#endif
#endif
//_setup_profile_print_events__________________________________________________/
