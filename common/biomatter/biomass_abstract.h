...
, public extends_             Physical::Mass_common                              //101019_100819
, public CORN::Item // Enumeration                                               //090728
{
 protected:  // state (adjusts as mass changes) These could probably be moved to biomass abstract
   ...
   mutable   float64  decomposition_constant;
      // Note that the decomposition rate constants are different for each model
      // and each organic matter type!

   // 100813 the decomposition constant should be a parameter/property (needs to be moved)
   // of the decomposition class which is separate from the biomass.
...
*/
 public:
   ...
   Biomass_abstract
      ...
      ,float64 decomposition_constant = 0.0);
      // Decomposition constant can be 0 for living matter                       //070726
 ...
 public:  // Accessors for mass components
   ...
   inline  virtual float64 get_decomposition_constant
      (Organic_matter_cycling cycling = OM_INTRINSIC_CYCLING)             const  //150629
 ...
 public: // modifiers
   ...
   inline float64 set_carbon_nitrogen_ratio (float64 new_CN_ratio) assignation_
                                { return carbon_nitrogen_ratio = new_CN_ratio; }
      // May return 0 if the biomass is empty
   inline float64 set_carbon_fraction (float64 new_C_fraction)     assignation_
                                    { return carbon_fraction = new_C_fraction; }
   RENDER_INSPECTORS_DECLARATION;                                                //151020BRC
};
//_Biomass_abstract_________________________________________________2006-03-16_/
#endif
