...
{  bool valid = CORN::Data_record::expect_structure(for_write);
   set_current_section("Explorer");                                              //190808
   //190808 expect_string("context",contextX);
   expect_directory_name("context",context_definition);                          //190808
   return valid;
}
...
{}
//_Contextualization::constructor_________________________________________2018_/
Contextualization::~Contextualization()
{
   delete active_directory;
   delete directory_record;
}
//_Contextualization::destructor__________________________________________2018_/
Directory_record &Contextualization::provide_directory_context() provision_
{
   if (!directory_record)
   {
      ...
      if (directory_record->context_definition.empty())                          //190809
         assert(false); // NYI infer context definition
      /*190809 now infor context definition
      if (directory_record->contextX.empty())
   ...
   // Derived classes may override this to do more definitive evaluation.
         get_specified_context(directory_record->contextX);
      }
      */
   }
...
,const CORN::OS::Directory_name  *specified_dir)                    affirmation_
{
   ...
   */
   std::string context_dirname;                                                  //190809
   std::wstring dirname(dir.get_name());
   CORN::wstring_to_string(dirname,context_dirname);                      //190809
   return (context_dirname != monument)                                          //190809
...
{  bool written = false;
   ...
   {
      CORN::OS::File_name_concrete directory_filename
         (get_active_directory(),".directory");
      CORN::VV_File directory_file(directory_filename.c_str());
...
{  // derived class will typically override
   CORN::wstring_to_string(get_active_directory().get_name(),context);
   return true;
}
