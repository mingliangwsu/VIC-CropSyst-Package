...
**********************************************************************/
  ...
  if (new_snow > 0.) density_new = new_snow_density(Tair);
  else               density_new = 0.0;
...
//  Tavg = (snow->surf_temp+Tgrnd)/2.0+KELVIN;
  ...
  if (options.SNOW_DENSITY == DENS_SNTHRM) {
    if (new_snow > 0.) {
      if (snow->depth>0.0) density = snow->density;
    ...
    if ( depth > 0. ) {
      overburden  = 0.5 * GRAVITY * RHO_W * swq;
...
**********************************************************************/
  ...
  /** Aged Snow **/
    ...
    if ( options.SNOW_ALBEDO == SUN1999 ) {
      ...
      if ( depth > 0.025 )              albedo = 0.5 + ( albedo - 0.5 ) * exp( -0.01 * fraction_day);// Use deep snow albedo
      else if ( cold_content < 0.0 )    albedo -= 0.006 * fraction_day;// Use shallow dry snow albedo
      else                              albedo -= 0.071 * fraction_day;// Use shallow melting snow albedo
      if ( albedo < 0 ) albedo = 0;
    } else {
      /* Accumulation season */
      if ( cold_content < 0.0 && !MELTING )
        albedo = NEW_SNOW_ALB*pow(param.SNOW_ALB_ACCUM_A, pow((double)last_snow * fraction_day, param.SNOW_ALB_ACCUM_B));
      /* Melt Season */
      else
        albedo = NEW_SNOW_ALB*pow(param.SNOW_ALB_THAW_A,  pow((double)last_snow * fraction_day, param.SNOW_ALB_THAW_B));
    }
  } else {
    albedo = 0; /* No snow falling or present */
  }
  return(albedo);
}
