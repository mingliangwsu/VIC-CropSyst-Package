...
{  if (rotation)
   {  CropSyst::Sowing_event *planting_event = new CropSyst::Sowing_event();
      ...
      /*
      const CORN::OS::Directory_name *parent_path                                //190114
         = parameter_form->ref_parameter_filename().get_path_directory_name();   //190114
      */

      const CORN::OS::File_name_concrete rot_filename
            (parameter_form->get_parameter_filename_prompt_if_necessary());
      const CORN::OS::Directory_name *parent_path  = rot_filename.get_path_directory_name();   //190114
      plant_event_form->bind_to
         (planting_event,parent_path,parameter_form->provide_version());         //190114
...
/*
         const CORN::OS::File_name &rot_filename = parameter_form->ref_parameter_filename(); //190114
*/
         const CORN::OS::File_name_concrete rot_filename
            (parameter_form->get_parameter_filename_prompt_if_necessary());
         const CORN::OS::Directory_name *parent_path  = rot_filename.get_path_directory_name();   //190114
         ...
         plant_event_form->bind_to
            (plant_event,parent_path,parameter_form->provide_version());         //190114
...
void __fastcall Trotation_editor_form::button_sortClick(TObject *Sender)
{
   ...
   listbox_rotation->bind_to(&(rotation->planting_events));
...
void __fastcall Trotation_editor_form::button_buildClick(TObject *Sender)
{
   ...
   {  if (listbox_rotation->Selected[i])                                         //020514
      {  CropSyst::Sowing_event *rot_entry = dynamic_cast<CropSyst::Sowing_event *>
         ...
         if (rot_entry)                                                          //990223
         {  CORN::Date_clad_32 date(rot_entry->begin_sync);                      //180326_990223
            ...
            CORN::Date_clad_32 new_entry_date(new_year,date.get_DOY());          //180326_990223
            ...
            {  CropSyst::Sowing_event *new_rot_entry = new CropSyst::Sowing_event();
			   ...
			   */
               ...
               new_rot_entry->begin_sync.sync_mode_clad.set(ACTUAL_DATE_MODE);   //020910
