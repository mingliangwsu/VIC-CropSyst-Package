...
{  unsigned long phrase_count = 0;
   ...
   Bidirectional_list translation_files_ANSI;
   Bidirectional_list translation_files_Unicode;
...
{  unsigned long phrase_count = 0;
   ...
   {  std::getline(translation_file,line);
      ...
      {  size_t delimiter_pos = line.find(" ");
         ...
         {  std::string language_code(line.substr(0,delimiter_pos));
            std::string translated_text(line.substr(delimiter_pos+1));
...
*/
      ...
      if (translation)
      {
         ...
         if (delimiter_pos != std::string::npos)
         {  std::wstring w_language_code(line.substr(0,delimiter_pos));
            std::string language_code;
            wstring_to_string(w_language_code,language_code);
            #ifdef _UNICODE
            std::wstring translated_text(line.substr(delimiter_pos+1));
            #else
            std::wstring translated_text_Unicode(line.substr(delimiter_pos+1));
