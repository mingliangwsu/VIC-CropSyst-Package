...
#define ICE_WATER_CONTENT_RELEVENCE_UNKNOWN_140822 0.0
   // ICE_WATER_CONTENT_RELEVENCE_UNKNOWN_140822 indicates that
   // it was not known whether ICE water content is relevent
   // and it was presume to be effectively irrelevent.


#define PROFILE_LAYER_INDEX 0
// Since layer 0 is not used layer value 0 represents the entire profile.
namespace Soil {                                                                 //181206
//______________________________________________________________________________
interface_ Hydraulic_properties_interface
{
 public:
   virtual ~Hydraulic_properties_interface()                                  {} //200325
 public: // may be computed by pedotransfer functions
   ...
   virtual float64 get_saturation_water_content_volumetric
      ( nat8 layer,float64 current_ice_water_content)                   const=0;
...
//181206class Soil_hydrology_interface
{
 public: // Soil profile state accessors
   inline virtual ~Hydrology_interface() {}                                      //170217
   ...
   virtual float64 get_water_plus_ice_content_volumetric_at
      (nat8 layer,CORN::Hour hour)                                      const=0;
   virtual float64 get_water_plus_ice_content_volumetric   (nat8 layer) const=0;
   virtual float64 get_water_filled_porosity
      ( nat8 layer, CORN::Hour for_hour)                                const=0;  // for_hour=24 for daily value
   virtual float64 get_water_potential                     (nat8 layer) const=0;
   /*181105 conceptual
   virtual float64 provide_water_total_to_layer
      ( bool all_water, nat8 layer) provision_;
   */

   virtual float64 provide_water_depth_profile(bool all_water)     provision_=0; //130930
   /*181105 conceptual
      { return provide_water_total_to_layer(all_water,PROFILE_LAYER_INDEX); }    //181105
      // This method is deprecated, use provide_water_total_to_layer (provided as an alias)
   */

   virtual float64 get_plant_avail_water( nat8 layer)                   const=0;
      // Need to check if this is volumetric or depth
   inline virtual float64 calc_water_depth_profile(bool all_water)      const    //181105_170601
      { return calc_water_depth_to_layer(all_water,PROFILE_LAYER_INDEX); }       //181105
      // This method is deprecated, use calc_water_depth_to_layer (provided as an alias)

   virtual float64 calc_water_depth_to_layer(bool all_water, nat8 layer) const=0; //181105
   virtual float64 calc_water_depth_to_depth(bool all_water, float32 depth=9999) const=0; //181105

   virtual float64 calc_available_water_fraction(nat8 layer, bool real_wilt_based) const=0; //160601
   virtual float64 calc_available_water_depth_to_depth(float64 depth=9999)               calculation_=0; //181105
   virtual float64 calc_available_water_depth_to_layer(nat8 layer = PROFILE_LAYER_INDEX) calculation_=0; //181105
   virtual float64 calc_available_water_depth_to(float64 depth)     calculation_ //160601
      // calc_available_water_depth_to deprecated
      { return calc_available_water_depth_to_depth(depth); }                     //180805
   ...
   virtual float64 *get_water_content_volumetric_liquid_profile                  //190119
      //190119 renamed get_liquid_water_content_layers_volumetric                   //140527
 ...
 public: // processes
   virtual float64 extract_water_mm
      (float64 water_uptake_mm[] , nat8 start_layer)                         =0;//140624
      // takes values in mm and returns mm
   virtual float64 extract_water
      (float64 water_uptake_m[] , nat8 start_layer)                          =0;
 ...
 public:  // process daily
   ...
   virtual Hydrology_interface *copy(float32 fraction)                  const=0; //140610
