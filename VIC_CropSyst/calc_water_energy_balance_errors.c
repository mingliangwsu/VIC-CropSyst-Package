...
***************************************************************/
  ...
  if (rec < 0) {
    ...
    if (rec != 0) error = inflow - outflow - (storage - last_storage);
    else error = 0.0;                                                            //150922LML do not check for spin-up period or the first day.
    cum_error += error;
    if((fabs(error) > fabs(max_error)) && (fabs(error) > 1e-5)) {
      max_error = error;
      fprintf(stderr,"Maximum Moist Error:\trec(%i)\terror(%.5f)\tcum_error(%.5f)\tinflow(%.5f)\toutflow(%.5f)\tnew_storage(%.5f)\told_storage(%.5f)\n",
          rec,error,cum_error,inflow,outflow,storage,last_storage);
    }
    if(rec == Nrecs-1) {
