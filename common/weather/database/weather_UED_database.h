...
public: // temporarily public so I could get the filename for a debug
   mutable
      UED:: // CS_UED::
      Database_file_indexed *database_file;                                      //060713
   bool owns_database;
...
#endif
//abandoned   contribute_ Climate_adjustments *ref_climate_adjustments; //not owned         //181011
public:
   Weather_UED_database
      ...
      ,const File_name *location_filename_to_provide_geolocation =0);            //091117
   Weather_UED_database                                                          //101008
      (UED::Database_file_indexed *database                                      //200329_170329
      ,bool take_database
      ,CORN::Quality_code attribute = CORN::unknown_quality
      ,const File_name *location_filename_to_provide_geolocation =0);  //091117
...
/*abandoned
   inline bool know_climate_adjustments(Climate_adjustments *ref_climate_adjustments_) contribution_
      {  ref_climate_adjustments = ref_climate_adjustments_;
         return true; }
*/
 ...
 public: // get recorded data for a date
   ...
   */
   ...
   /*200327 not used
   virtual float64 get__isothermal_longwave_net_radiation(weather_database_get_formals) const ;//070204
   */
 ...
 public: // set data methods
   ...
   virtual float64 set_cloud_cover_percent
      (const CORN::Date_const &date_time,float64 cloud_cover_percent
      ,CORN::Quality &attribute);  //991022
   /*200327 not currently used
   virtual float64 set__isothermal_longwave_net_radiation      (const CORN::Date_const &i_date_time,float64 isothermal_longwave_net_radiation  ,CORN::Quality &attribute);                  //070203
   */
   virtual float64 set_net_radiation
      (const CORN::Date_const &i_date_time,float64 net_radiation
      ,CORN::Quality &attribute); //070203
   virtual float64 set_pot_evapotranspiration_FAO_Penman_Monteith_reference_mm
      (weather_database_set_formals);                                            //070525
   virtual float64 set_avg_fullday_vapor_pressure_deficit_kPa
      (const CORN::Date_const &date_time,float64 precip_mm,CORN::Quality &attribute); //011009
//NYI/*000612*/     virtual float64 set_max_vapor_pressure_deficit_kPa  (const CORN::Date_time &i_date_time,float64 precip_mm            ,CORN::Quality_code &attribute);
   ...
   #endif
   inline virtual void set_override_safety(bool override_set_safety_) modification_
      { override_set_safety = override_set_safety_;}                             //050525
   inline UED::Database_file_indexed &mod_database_file()          modification_
      {return *database_file; }                                                  //070724
   inline const UED::Database_file_indexed &ref_database_file()            const
      {return *database_file; }                                                  //161212
 public: // delegate overrides
   inline const UED::Geolocation_record *ref_geolocation_record()          const
      { return database_file->ref_geolocation_record(); }                        //050429
   inline UED::Record_base *goto_next(Record_code record_type)
      { return database_file->goto_next_record_of_type(record_type); }           //991210
   inline virtual datetime64 get_earliest_date_time                              //970915
      ( Record_code  for_record_type_code = UED_FOR_ALL_RECORD_TYPES
      , UED::Variable_code for_var_code = UED_FOR_ALL_VARIABLES)
      { return database_file->get_earliest_date_time(for_record_type_code,for_var_code); }
   inline virtual datetime64 get_latest_date_time                                //970915
      ( Record_code  for_record_type_code = UED_FOR_ALL_RECORD_TYPES
      , UED::Variable_code for_var_code = UED_FOR_ALL_VARIABLES)
         { return database_file->get_latest_date_time(for_record_type_code,for_var_code); }
   inline virtual CORN::date32 get_earliest_available_date()               const
      { return database_file->get_earliest_date_time
         (UED_FOR_ALL_RECORD_TYPES,UED_FOR_ALL_VARIABLES); }                     //140822
...
//_Weather_UED_database________________________________________________________/
#endif
