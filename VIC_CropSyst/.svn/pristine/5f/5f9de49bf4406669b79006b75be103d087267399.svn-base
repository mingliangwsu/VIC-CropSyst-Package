               ...
               soil_con_struct  *soil_con)
{
  ...
  //*NetLongSnow = 0;
  //fprintf(stderr,"has set *NetLongSnow = 0!!!\n");
  ...
  //fprintf(stderr,"rec %d Qnet %f RefreezeEnergy %f sensible_heat %f latent_heat %f latent_heat_sub %f NetShortSnow %f NetLongSnow %f LongSnowIn %f\n",rec,Qnet,RefreezeEnergy,sensible_heat,latent_heat,latent_heat_sub,NetShortSnow,*NetLongSnow,LongSnowIn);
...
//        snow->surface_flux *= -( snow->surf_water / snow->vapor_flux );
    ...
    /* Else, SnowPackEnergyBalance(T=0.0) <= 0.0 */
      ...
      if (SurfaceSwq > MIN_SWQ_EB_THRES) {
        snow->surf_temp = root_brent((double)(snow->surf_temp-param.SNOW_DT),
...
/*     snow->vapor_flux   = 0.0; */
  ...
  //fprintf(stderr,"Qnet %f snow->surf_temp %f \n",Qnet,snow->surf_temp);
