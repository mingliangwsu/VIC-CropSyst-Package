...
,const Hydrology_interface            &soil_hydrology_                           //061004
,const Structure                      &soil_structure_                           //070725
...
//_Temperature_profile::constructor_________________________________1997-07-30_/
bool Temperature_profile::initialize
...
//_initialize_______________________________________________________2001-02-07_/
float64 Temperature_profile::profile_water_amount()
{  float64  sum = 0.0;
...
//_profile_water_amount________________________________________________________/
float64 Temperature_profile::damping_depth()    // m
{
...
//__damping_depth_________________________________________________________1998_/
void Temperature_profile::find_soil_temp
...
,float64   plant_height)                                                         //990202
{
   ...
   surface_temperature = calc_actual // C  //181212 actual_surface_temp  //C
      (max_temp,min_temp
      ,solar_rad
      ,snow_pack_thickness
      ,leaf_area_index
...
//_find_soil_temp_________________________________________________________1998_/
float64 Temperature_profile::base_line_temperature
(nat8       layer  // sublayer
...
{  float64 DD = damping_depth();                                                 //990213
   ...
   if (!CORN::is_zero<float64>(DD/*200127 ,0.0,0.00000001*/))                    //981027
   {  float64 depth_layer = soil_layers.get_depth_m(layer);                      //981027
      float64 ZD =  CORN_must_be_less_or_equal_to(((layer == 1)                  //991021
       ...
       : (layer == (soil_layers.count() + 1))                                    //991021
...
//_base_line_temperature____________________________________________1999-01-20_/
void Temperature_profile::long_term_initialization
...
//_long_term_initialization_________________________________________1999-02-13_/
float64 Temperature_profile::find_heat_source_sink  // J/m2/s
...
//_find_heat_source_sink_______________________________________________________/
float64 Temperature_profile::albedo_for_soil_temp()
...
//_albedo_for_soil_temp________________________________________________________/
...
//_calc_longwave_net_radiation_for_soil_temp________________________2015-10-22_/
float64 Temperature_profile::radiation_load         // J/(m2 sec)
...
,float64 fract_residue_interception)
{
   ...
   float64  rad_load = (meteorological.get_snow_pack_thickness() > 0)            //141205_991004
   ? 0.0
...
//_radiation_load______________________________________________________________/
void Temperature_profile::SolveTemp
...
,const   Residues_interface *residue)                                            //050726
{
   ...
   for (nat8  sl_tc = 1 ; sl_tc <= soil_layers.count(); sl_tc++) // Note this is different from Basic Version
...
#define           min_cover_height 0.1
   ...
   nat8  num_layers = soil_layers.count();
...
//_SolveTemp___________________________________________________________________/
void  Temperature_profile::get_temperatures
...
#endif
   nat8  number_layers = soil_layers.count();                                    //061004
         ...
         {  float64 temperature_sl = new_temperatures[layer];
            ...
            {  uint16 iteration = 1;
               ...
               {  float64 Teq = (Thigh + Tlow) / 2.0;
                  ...
                  heat_source_sink[layer] =
                  find_heat_source_sink
                     (thickness_sl, new_ice_content[layer], ice_content[layer]);
                  SolveTemp
                  (weather
                  ,plant_height
                  ,fract_crop_interception
...
//_get_temperatures_________________________________________________1999-01-20_/
RENDER_INSPECTORS_DEFINITION(Temperature_profile )
{
   #ifdef CS_OBSERVATION
   if (
      #if ((CS_VERSION > 0) &&(CS_VERSION < 6))
        (CS::optation_global.is_desired(CSVC_soil_base_temperature_time_step_avg)) ||
      #endif
        (CS::optation_global.is_desired("temperature")))
      inspectors.append(new CS::Inspector_vector
      (temperatures,soil_layers.count()
      ,UC_Celcius,*context,"temperature",value_statistic
      ,CSVC_soil_base_temperature_time_step_avg));
   #endif
   return 0;
}
// _RENDER_INSPECTORS_DEFINITION_Temperature_profile________________2019-01-19_/
