...
#if SPATIAL_SNOW
double calc_snow_coverage(const double max_snow_distrib_slope,
                          const double old_coverage_orig,
                          const double swq,
                          const double old_swq,
                          const double depth,
...
{
  double coverage = 0;
  double old_coverage = old_coverage_orig;
  ///*************************************
  ...
  if ( snowfall[WET] > 0 ) {
    ...
    if (is_approximately(store_swq,0)) {
      ...
      //occurs.
      double old_max_snow_depth = max_snow_depth;
      max_snow_depth = 2. * depth;
      if ((max_snow_depth < old_max_snow_depth) || is_approximately(old_max_snow_depth,0)) {
