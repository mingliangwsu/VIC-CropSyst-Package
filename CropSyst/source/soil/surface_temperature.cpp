...
//_Soil_surface_temperature____________________________________________________/
float64 Soil_surface_temperature
::calc_actual
(float64 max_temp
,float64 min_temp
...
{  float64 residue_mass_kg_ha = (residue_mass / 0.0001);  // convert per m2 to per ha
   ...
   float64 mean_surface_temp_residue = calc_mean
     (max_surface_temp_residue
     ,0.43
     ,min_surface_temp_residue
     ,0.57);
   float64 max_surface_temp_crop
      = max_temp + (0.437209 * max_temp) * exp(-0.86102 * leaf_area_index);      //990611
   float64 min_surface_temp_crop =min_temp + 0.057832*leaf_area_index + 0.95832; //990611
   float64  mean_surface_temp_crop = calc_mean //181212 mean_surface_temp
