...
static char vcid[] = "$Id: redistribute_during_storm.c,v 5.1.2.1 2007/04/18 20:18:59 vicadmin Exp $";
...
**********************************************************************/
  ...
  double        temp_wet;
  double        temp_dry;
  int           num_snowbands = options.SNOW_BAND;
  #if VIC_CROPSYST_VERSION>=3
  if (is_crop) num_snowbands = 1;
  #endif

  /** Redistribute Soil Moisture **/
  for(layer = 0; layer < options.Nlayer; layer++) {

    for(band = 0; band < num_snowbands/*LML 150428 options.SNOW_BAND*/; band++) {
