...
(const CORN::OS::Directory_name *dir)                               affirmation_
{
   ...
   bool beneath_Output     = is_beneath("*utput*",dir);                          //181219
   bool contains_Output  = has_monument("*utput*",dir);                          //181219
/*
   return !is_beneath("*Output",dir)                                             //180406
      &&  (has_monument("*Output",dir)                                           //180406
*/
   return (!beneath_Output && contains_Output)                                   //181219_180404
         ||  !has_subscenarios;
}
...
(const CORN::OS::Directory_name *dir)                               affirmation_
{  return
   context_is_scenarios                                                          //181218
   ||(    is_underneath("Scenarios",dir)
...
//std::clog << "level_or_ultimate_scenario:" << level_or_ultimate_scenario.c_str() << std::endl;
   ...
   else
   {
      ...
      CORN::Seclusion potential_ultimate_pattern
         ("*Output",true,false);
      potential_ultimate_pattern.include_pattern("*");
