...
, application_directory (0),  application_directory_owned(false)
...
Program_directory::~Program_directory()
{  if (suite_directory_owned)       delete suite_directory;
   if (application_directory_owned) delete application_directory;
}
//______________________________________________________________________________
const //141119
...
{  if (!suite_directory)
   {  // It is possible this program is in the suite directory
      ...
      if (parent_dir)
      {
         ...
         suite_directory = new Suite_directory(*parent_dir);                     //140104
         if (!suite_directory->is_valid())                                       //120112
         {  // It isn't so try assuming the parent is an application directory in a suite directory
            delete suite_directory;                                              //120112
            suite_directory = 0;                                                 //120112
            provide_application_directory();
            if (application_directory)
               suite_directory =
                  const_cast<Suite_directory *>                                  //141119
                  (application_directory->provide_suite_directory_const());      //141119
...
{  if (! application_directory)
   {
      ...
      if (parent_dir)                                                            //140104
         application_directory = new Application_directory(*parent_dir,0 );      //140104_120319
