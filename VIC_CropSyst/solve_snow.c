...
double solve_snow(char                 overstory,
          ...
          double               dp,
          double               mu,
          double               prec, /*LML note: ppt over WET fraction*/
          ...
          double              *melt_energy,
          double              *out_prec, /*LML note: over WET part*/
          double              *out_rain, /*LML note: over WET part*/
          double              *out_snow, /*LML note: over WET part*/
          double              *ppt,
          double              *rainfall,
          double              *ref_height,
          double              *roughness,
          ...
          //150929LML,crop_data_struct    *current_crop                                     //150702LML crops
          ,int                 veg_class_code
...
*********************************************************************/
  ...
  //151001LML extern veg_lib_struct *veg_lib;
  #ifdef VIC_CROPSYST_VERSION
  veg_lib_struct &veglib = *veg_con[iveg].VCS.veg_lib[band];
  #else
  ...
  month       = dmy/*150609 [rec]*/->month;
  hour        = dmy/*150609 [rec]*/->hour;
  day_in_year = dmy/*150609 [rec]*/->day_in_year;
  ...
  if ( (snow->swq > 0 || snowfall[WET] > 0.
    ...
    *****************************/
      ...
      ****************************************/
      if ( overstory ) {
        ...
        ***********************************************/
        ...
        ErrorFlag = snow_intercept((double)dt * SECPHOUR,
                                    1.,
                                    veglib.LAI[month-1],
                                    (*Le),
                                    longwave,
                                    LongUnderOut,
                                    veglib.Wdmax[month-1],
                                    ShortOverIn,
                                    *ShortUnderIn,
                                    Tcanopy,
                                    BareAlbedo,
      ...
      else if(snowfall[WET] > 0. && veg_var_wet->Wdew > 0.) {
    ...
    if ( snow->swq > 0.0 || snowfall[WET] > 0 ) {
      /******************************
