...
//_Tabular_AdvGridExcelIO___________________________________________2005-04-10_/
...
{  CORN::Tabular_file::get(data_rec);
   ...
   //NYI 050426    if (file_extent.end_marker.length() && record_line_string.find(file_extent.end_marker) != NPOS)
...
//_Tabular_AdvGridExcelIO::get______________________________________2005-04-06_/
bool Tabular_AdvGridExcelIO::get_next(Data_record &data_rec)
{  bool got = get(data_rec);
   return got;
}
//_Tabular_AdvGridExcelIO::get_next_________________________________2005-04-06_/
...
//_Excel_workbook_with_TDF_convertor___________________________________________/
...
, tabular_file(0)
{}
//_Excel_workbook_with_TDF_convertor:constructor_______________________________/
CORN::Tabular_file *Excel_workbook_with_TDF_convertor::
provide_tabular_file(Year &file_year_not_applicable)
{
   CORN::OS::File_name_concrete target_filename_temp(arguments.target_filename);
   if (!tabular_file) tabular_file = new Tabular_AdvGridExcelIO
      (target_filename_temp,grid_workbook,grid_Excel_IO);
   return tabular_file;
}
