...
static char vcid[] = "$Id: check_state_file.c,v 4.7 2006/10/18 20:58:57 vicadmin Exp $";

FILE *check_state_file(char *init_state_name, int *startrec)
/*********************************************************************
...
*********************************************************************/
{
  ...
  *startrec = 0;
  ...
  if ( tmp_Nlayer != options.Nlayer ) {
    sprintf(ErrStr,"The number of soil moisture layers in the model state file (%d) does not equal that defined in the global control file (%d).  Check your input files.", tmp_Nlayer, options.Nlayer);
    nrerror(ErrStr);
  }
  if ( tmp_Nnodes != options.Nnode ) {
    sprintf(ErrStr,"The number of soil thermal nodes in the model state file (%d) does not equal that defined in the global control file (%d).  Check your input files.", tmp_Nnodes, options.Nnode);
    nrerror(ErrStr);
  }
