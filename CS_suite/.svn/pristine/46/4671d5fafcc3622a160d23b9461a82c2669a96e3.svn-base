...
//200815    Command_options_data_source    &command_line_options_                //160829
,const CS::Identification       &ID_
,bool                            CPF_mode_)                                      //151026
: Simulation_engine     (arguments_)                                             //200817
//200815: Simulation_engine     (command_line_options_)                                  //160829_160222
, ID                    (ID_)                                                    //151028
, template_scenario     (CPF_mode_)                                              //150513
, geolocation           ("geolocation")                                          //200115
...
bool Cycler::initialize()                                       initialization_
{
   ...
   if (CORN::global_arguments->verbose_flag) //creation                          //200115
   //200115 if (verbose) //creation
   {
      std::clog
         << "info: creating directory" << std::endl
         << "dir=  " << get_output_directory_name().c_str() << std::endl;
   }
   CORN::date32 hold_today(simdate_raw);                                           //170525_170220
   ...
   CropSyst::Rotation *rotation // There is not necessarily a rotation
   ...
   {  Cycled_scenario_simulation *cycled_scenario = render_scenario_simulation(cycle);
      ...
      {  CORN::Unidirectional_list cycled_plantings;
         if (CORN::global_arguments->verbose_flag)  // actually use verbosity level or program-entry            //200115
         //200115          if (verbose)   // level or program-entry
         ...
         while ((cycled_planting = dynamic_cast<CropSyst::Sowing_event *>(cycled_plantings.pop_at_head())) != 0)
   ...
   today_driving_raw = hold_today;                                               //170525_170220
...
/*
scenario cycles now have have an extension 'cycle'
this will denote cycle directories as monads this will disqualify these as consituents
(actually because cycle directories are already disqualified because
they are under the disqualified output directory (which is itself a monad)
*/
...
{  // We disable the cropping system mode because we generate from
   ...
   scenario_control_and_model_options.cropping_system_mode_clad.set(rotation_cropping_system);
   return true;
}
...
render_scenario_simulation(nat32 cycle_shift_)                      //rendition_
{
   ...
   return new Cycler::Cycled_scenario_simulation
      ...
      ,get_output_directory_name());                                             //170225
}
...
{  if (!weather_DB_daily)
   {
      ...
      weather_DB_daily = new Weather_database_daily_UED
           (template_scenario.weather_filename,simdate_raw                         //170525
