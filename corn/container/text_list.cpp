...
Text_list::Text_list()
{}
//_Text_list:default_constructor_______________________________________________/
Text_list::Text_list
...
, char separator_character)                                                      //090919
{
   ...
   {  size_t start_pos = 0;
      ...
      {  if ((initial_text[i] == separator_character) || (initial_text[i+1] == 0))
         {  if (initial_text[i+1] == 0) run_length +=1; // at the end of string  //130103
            if (run_length)
...
//_Text_list:constructor_______________________________________________________/
Text_list::Text_list
...
,wchar_t separator_character)
{
   ...
   {  size_t start_pos = 0;
      ...
      for (int i = 0; initial_text[i] != 0; i++)
      {  if ((initial_text[i] == separator_character) || (initial_text[i+1] == 0))
         {  if (initial_text[i+1] == 0) run_length +=1; // at the end of the string
            if (run_length)
               append(new Item_wstring(initial_text, start_pos, run_length ));
            start_pos = i+1;
            run_length = 0;
         } else run_length += 1;
      }
...
//_Text_list:constructor(wide)______________________________________2018-08-06_/
Text_list::Text_list
(const Text_list &copy_from)
{  copy(copy_from);
}
//_Text_list:constructor____________________________________________2017-01-01_/
...
//_copy_____________________________________________________________2017-01-01_/
bool Item_string::write(std::ostream &strm)                           stream_IO_ //150728
...
{  if (is_empty()) return 0;                                                     //030709
   ...
   {
      std::string item_string; item->append_to/*200624_string*/(item_string);              //180115
...
//_add_cstr_________________________________________________________2006-09-10_/
Item_wstring *Text_list::add_wstr(const wchar_t *text_to_add)
{
...
//_add_wstr_________________________________________________________2012-02-09_/
Item_string
*Text_list::add_string (const std::string &text_to_add)                          //130123
{  return add_cstr(text_to_add.c_str()); }
//_add_string_______________________________________________________2012-01-23_/
Item_wstring
*Text_list::add_wstring(const std::wstring &text_to_add)                         //130123
{  return add_wstr(text_to_add.c_str()); }
//_add_wstring______________________________________________________2012-01-23_/
/* conceptual
...
//_find_wstring_____________________________________________________2013-03-04_/
...
//_find_string______________________________________________________2013-03-10_/
