...
{}
//_Convertor_weather:constructor____________________________________2015-12-29_/
Convertor_weather::~Convertor_weather()
{  FOR_EACH_IN(station_database,Station_database,station_databases,each_station_db)
      station_database->ued_file.finalize();                                     //191010
   FOR_EACH_END(each_station_db)
}
//_Convertor_weather:destructor_____________________________________2015-12-29_/
...
{  bool loaded_any = false;
   ...
   {  // Automatically setup any listed UED files.
      CORN::SDF_Clad UED_filenames(parameters.UED_filenames.c_str());
      FOR_EACH_IN(ued_filename, Item,  UED_filenames,each_UED_filename)
      {
...
//_setup_stations_from_response_UED_filenames_______________________2008-01-08_/
void Convertor_weather::notify_climgen()
{  setdown_climgen_options();
}
//_notify_climgen___________________________________________________2009-08-27_/
...
//_provide_station_database______________________________________________2005?_/
...
//_provide_station_database_from_UED_filename_______________________2008-01-08_/
...
//_done_with_station________________________________________________2014-06-09_/
#ifdef NYI
...
{  Air_temperature_maximum                   air_temperature_max;
   ...
   CORN::statistical::Sample_cumulative<float32,float32> Tavg_avg[13]; // index 0 not used
   ...
   CORN::statistical::Sample_cumulative<float32,float32>Tavgs;
   for (int m = 1; m <= 12; m++)
...
//_calc_TAV_and_AMP_________________________________________________2014-09-11_/
...
//_render_atmospheric_CO2_change____________________________________2015-03-30_/
...
//_get_recognized_creation_layout___________________________________2016-02-12_/
...
//_is_import_data_record____________________________________________2016-02-12_/
...
//_provide_UED_file_________________________________________________2016-02-12_/
...
{  // May return 0 if station ID is not stored in the data record
   nat32  station_ID_or_name_column_num = format.ID_column_number;     // first check if there is a station_ID_column specified
   if (!station_ID_or_name_column_num)
        station_ID_or_name_column_num = format.name_column_number;  // if not check if there is a station_name specified
   if (station_ID_or_name_column_num)
   {
   ...
   {   // The format doesn't specify which column has the station ID
      ...
      {
         switch (data_column->TC_layout.data_type_clad.get()) // data_type)
         {
...
//NYN                           string column if there are none others with ID).
   ...
   return
      record_station_ID_result.length()                                          //100614
      ? record_station_ID_result.c_str()                                         //100614
      : 0;                                                                       //100614
}
//_get_station_ID_from_data_record__________________________________2016-02-12_/
   ...
   // this should actually be done in ClimGen
         */
         ...
         FOR_EACH_IN(data_column,Import_column ,data_rec.tabular_data_columns,each_column)  //050802
         {
               ...
               /*160212  This was a temporary hack to provide RHmax and or RHavg
                  This functionality moved to to ClimGen (or relative humidity calculators).
               switch (data_column->TFFC_layout.variable_code_clad.get())              //140610
               {
...
//_set_validated_elements___________________________________________2016-02-12_/
...
//_inject_calculated_values_________________________________________2016-02-12_/
Geolocation_record  *Convertor_weather::provide_geolocation_record()
{  return station_database
      ? station_database->ued_file.Database_file::provide_geolocation_record()
      : 0;
}
//_mod_geolocation_record___________________________________________2016-02-12_/
} // namespace UED /
