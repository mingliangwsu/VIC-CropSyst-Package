...
**********************************************************************/
{
  ...
  if(options.ARC_SOIL) {
    ...
    temp = read_soilparam_arc(soilparam, soilparamdir
      #ifndef VIC407
      , &Ncells, RUN_MODEL, *cell_cnt
      #endif
      );
    *cell_cnt++;
    ...
    if(!(*MODEL_DONE) && (*RUN_MODEL))
    {
...
#ifdef CONST_SOIL_MIDDLE_LAYER_DEPTH
        if (layer < (options.Nlayer - 1)) temp.depth[layer] = 0.1;              //180807LML
#endif
      } // for layer
      ...
      if (options.ORGANIC_FRACT) {
        ...
        for(layer = 0; layer < options.Nlayer; layer++) {
          ...
          if(temp.organic[layer] > 0 && temp.bulk_dens_org[layer]>=temp.soil_dens_org[layer]) {
            sprintf(ErrStr,"WARNING: layer %d organic bulk density (%f) must be less than organic soil density (%f)", layer, temp.bulk_dens_org[layer], temp.soil_dens_org[layer] );
            temp.bulk_dens_org[layer] = temp.soil_dens_org[layer];
            //190116LML nrerror(ErrStr);
          }
#endif /* !OUTPUT_FORCE */
        }
      ...
      if (veg_lib)   //150126 Added by Roger Nelson to allow reading soil files without needing global parameter file
      {
      for (j=0; j<12; j++) {
      ...
      if (options.FROZEN_SOIL) {
        std::clog << "Automaticlly set frozone soil active for all gridcells!\n";
        temp.FS_ACTIVE = TRUE;
      }
      ...
      /********************************************************
      Keyvan added this ******************* May092014
      to read clay content fron input file
      because CropSyst is sensetive to soil charchteristics
      ********************************************************/
            /* read layer clay content */

      for(layer = 0; layer < options.Nlayer; layer++)
      {
        if (options.VCS.clay_input==1)
        {
          token = strtok (NULL, delimiters);
          while (token != NULL && (length=strlen(token))==0) token = strtok (NULL, delimiters);
          if( token == NULL ) {
            sprintf(ErrStr,"ERROR: Can't find values for CLAY CONTENT for layer %d in soil file\n", layer );
            nrerror(ErrStr);
          }// if ttoken
          sscanf(token, "%lf", &temp.VCS.clay[layer]);

          #if ! OUTPUT_FORCE

        if(/*150630LML options.FULL_ENERGY && */(temp.VCS.clay[layer] > 1. || temp.VCS.clay[layer] < 0)) {
            fprintf(stderr,"Need valid clay content as a fraction to run");
            fprintf(stderr," Full Energy model, %f is not acceptable.\n", temp.VCS.clay[layer]);
            exit(0);
          }
        }
          ...
          /******************************************
          if clay content is not provided caculate it soil clay content
          ******************************************/
          double clay_content= pow(10,(1-(temp.bulk_density[layer]/temp.soil_density[layer])-0.332+0.0007251*temp.quartz[layer]*100.0)/0.1276)/100.0; /*//150630LML for test only*/
          temp.VCS.clay[layer]=clay_content;                                         /*//150630LML for test only*/
      } // if claye
          #endif /* !OUTPUT_FORCE */
     } // for
#endif // CropSyst version
      #if (VIC_CROPSYST_VERSION >= 2)
     if(options.VCS.clay_input==1){

     //for_debuging#std::clog << "DEBUGGING SOIL !!!\n";
     //for_debuging#for(layer=0;layer<options.Nlayer;layer++) {
     //for_debuging#    if (layer < 3) {
     //for_debuging#       temp.VCS.clay[layer] = temp.VCS.clay[3];
     //for_debuging#       temp.quartz[layer] = temp.quartz[3];
     //for_debuging#    }
     //for_debuging#
     //for_debuging#}
     ...
     temp.VCS.S_max = calc_Maximum_sorptivity(temp.quartz[0] * 100.0,
      ...
      /*LML 141103 read CropSyst Soil ID */
      /*150723LML obsolete
      #if (VIC_CROPSYST_VERSION>=3)
      token = strtok (NULL, delimiters);
      while (token != NULL && (length=strlen(token))==0) token = strtok (NULL, delimiters);
      if( token == NULL ) {
        sprintf(ErrStr,"ERROR: Can't find values for CropSyst_Soil_ID in soil file\n");
        nrerror(ErrStr);
      }
      sscanf(token, "%d", &tempint);
      temp.CropSyst_Soil_ID = tempint;
      #endif
      */
      ...
      *******************************************/
      for(layer = 0; layer < options.Nlayer; layer++) {
        ...
        if (temp.Wpwp[layer] > temp.max_moist[layer]) {
            fprintf(stderr,"L:%d max_moist:%f < Wpwp:%f \n",layer,temp.max_moist[layer],temp.Wpwp[layer]);
...
//#if (VIC_CROPSYST_VERSION>=3)
        ...
        //temp.Wcr[layer] = Wcr_FRACT[layer] * temp.max_moist[layer];              //170215LML
        //temp.Wpwp[layer] = Wpwp_FRACT[layer] * temp.max_moist[layer];            //170215LML
//#endif
      } // for
      ...
      *******************************************/
      ...
      for(layer = 0; layer < options.Nlayer; layer++) {
        ...
        if(init_ice_fract[layer]>=temp.porosity[layer])
        {//excess ground ice present
      ...
      **********************************************/
      ...
      //161107LML added macro def condition
      ...
      ****************************************************************/
      /******** in this version all the soil parameters are re initialized using pedo transfer functions
      used in CropSyst,
      *********************************************************/


      #if (VIC_CROPSYST_VERSION >= 2)
      if(!options.VCS.clay_input==1) //190131RLN {
      #endif
      for(layer=0;layer<options.Nlayer;layer++) {
         #if (VIC_CROPSYST_VERSION >= 2)
         double clay_content= pow(10,(1-(temp.bulk_density[layer]/temp.soil_density[layer])-0.332+0.0007251*temp.quartz[layer]*100)/0.1276)/100;
         temp.VCS.clay[layer]=clay_content;
         //190806LML temp.Wcr[layer]  = Wcr_FRACT[layer]* temp.max_moist[layer]/0.7; //keyvan changed
                                                                         //190805LML Checked with Roger that he used (0.8 * field_capacity) to get Wcr fraction. So might need correct it.
         temp.Wcr[layer]  = Wcr_FRACT[layer]* temp.max_moist[layer]; //190806LML
         temp.VCS.Field_Capacity[layer] = temp.Wcr[layer] / 0.8;     //190806LML When roger create Wcr, he used 0.8.

         temp.Wpwp[layer] = Wpwp_FRACT[layer]* temp.max_moist[layer]; //keyvan changed- yield is very sensetive to Wpwp
         #else
         // Original VIC   150126RLN
...
//190131RLN      #if (VIC_CROPSYST_VERSION >= 2)
//190131RLN      } //if clay
//190131RLN      #endif
