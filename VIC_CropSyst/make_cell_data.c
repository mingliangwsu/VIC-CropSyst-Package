...
**********************************************************************/
{
  ...
  cell_data_struct **temp = (cell_data_struct**) calloc(veg_type_num, sizeof(cell_data_struct*));
  for (int i = 0; i < veg_type_num; i++) {
    temp[i] = (cell_data_struct*) calloc(options.SNOW_BAND, sizeof(cell_data_struct));
  }
  return temp;
}
