  ...
  if(param_set.FORCE_FORMAT[file_num] == BINARY){
    ...
    while ( !feof(infile) && (rec * param_set.FORCE_DT[file_num] 
      ...
      for(i=0;i<Nfields;i++) {
        ...
        //190721 LML incase the original prec is negative value. It may cause problem
        if (field_index[i] == PREC && forcing_data[field_index[i]][rec] > 800)
            forcing_data[field_index[i]][rec] = 0;
      }
