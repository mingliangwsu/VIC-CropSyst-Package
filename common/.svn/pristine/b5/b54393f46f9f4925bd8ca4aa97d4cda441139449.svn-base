...
,const File_name *location_filename_to_provide_geolocation)                      //091117
: Weather_database()
...
Weather_UED_database::Weather_UED_database
(//200327 CS_UED::
UED::Database_file_indexed *_database_file
,bool take_database
,CORN::Quality_code attribute
,const File_name *location_filename_to_provide_geolocation)                      //091117
...
,const File_name *location_filename_to_provide_geolocation)
{  bool UED_has_geolocation = database_file->ref_geolocation_record();
   if (location_filename_to_provide_geolocation
       && file_system_engine.exists(*location_filename_to_provide_geolocation))  //050817
...
Weather_UED_database::~Weather_UED_database()
{  database_file->finalize();                                                    //190717_171110
...
{  const Geolocation *geoloc_rec = database_file->ref_geolocation_record();
...
{  UED::Time_query_single_date time_qry
   ...
   actual_units = UC_meter;   float32 precipitation   = database_file->get_no_create(UED::Record_type_distinguisher_interface::UED_data_set_year_var_units ,UED::STD_VC_precipitation.get()             ,time_qry,actual_units,provider.precipitation_actual         );
   actual_units = UC_Celcius; float32 temperature_max = database_file->get_no_create(UED::Record_type_distinguisher_interface::UED_data_set_year_var_units ,UED::STD_VC_max_temperature.get()           ,time_qry,actual_units,provider.air_temperature_max   );
   actual_units = UC_Celcius; float32 temperature_min = database_file->get_no_create(UED::Record_type_distinguisher_interface::UED_data_set_year_var_units ,UED::STD_VC_min_temperature.get()           ,time_qry,actual_units,provider.air_temperature_min   );
   actual_units = UC_Celcius; float32 temperature_avg = database_file->get_no_create(UED::Record_type_distinguisher_interface::UED_data_set_year_var_units ,UED::STD_VC_avg_temperature.get()           ,time_qry,actual_units,provider.air_temperature_avg   );
/*181011 abandonded. actually I think this class is obsolete,
adjustment mechanism applied in CS_weather_daily_UED.cpp
   if (ref_climate_adjustments)                                                  //181011
   {
      precipitation     *= ref_climate_adjustments->precipitation_multiplier;
      temperature_max   += ref_climate_adjustments->temperature_addend;
      temperature_min   += ref_climate_adjustments->temperature_addend;
      temperature_avg   += ref_climate_adjustments->temperature_addend;
   }
*/
   provider.precipitation_actual.set_m                (precipitation    ,provider.precipitation_actual);
   provider.air_temperature_max.update_value_qualified(temperature_max  ,provider.air_temperature_max);
   provider.air_temperature_min.update_value_qualified(temperature_min  ,provider.air_temperature_min);
   provider.air_temperature_avg.update_value_qualified(temperature_avg  ,provider.air_temperature_avg);
...
/*200327 not used
float64 Weather_UED_database::get__isothermal_longwave_net_radiation(weather_database_get_formals) const
{  // Not yet implemented (Need to derive a cloud cover variable)
   UED::Time_query_single_date time_qry(date_time_.get_datetime64(),(UED_units_code)UT_day);
   UED_units_code actual_units = UC_percent;
   override_set_safety = false;                                                  //991013
...
//_get__isothermal_longwave_net_radiation____________________________1999-10-22_/
*/
...
/*200327 not currently uesd
float64 Weather_UED_database::set__isothermal_longwave_net_radiation
...
bool Weather_UED_database::enable_substitute_dates
(CORN::Year substitute_base_year
,CORN::Year simulation_start_year
,int8 substitute_year_offset_                                                    //161212
,bool allow_substitute_dates_)                                     contribution_
{
   CS_UED::Database_file_indexed *as_CS_UED
      = dynamic_cast<CS_UED::Database_file_indexed *>(database_file);
   return  as_CS_UED/*database_file*/->enable_substitution
   (substitute_base_year
   ,simulation_start_year
   ,substitute_year_offset_);
}
//_enable_substitute_dates_________________________200327_160407_160331_141031_/
