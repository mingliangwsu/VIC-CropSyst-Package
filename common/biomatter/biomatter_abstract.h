...
{private:
 ...
 public: // 'structors
   Biomatter_abstract
      (float64 carbon_fraction      =0                                           //070726
      ,float64 carbon_nitrogen_ratio=0);                                         //070726
...
public:
   ...
   float64  set_pH(float64 pH_)                 modification_{ return pH = pH_;}
   ...
   virtual modifiable_  Physical::Water &mod_water()            { return water;} //150215
      inline virtual float64 get_temperature_Celcius()                     const
                                          { return water.temperature.Celcius();} //150529
   inline virtual const float64 &ref_temperature_Celcius()                 const
                                    { return (water.temperature.ref_amount()); } //150529
   inline virtual float64 set_temperature_Celcius
      (float64 temperature_C_)                                     modification_
                        { return water.set_temperature_Celcius(temperature_C_);}
   virtual float64 set_mass_wet_at_percent_moisture                              //090729
      (float64 new_mass_wet,CORN::Units_code new_units
   ...
   virtual Biomatter_abstract *extract_volume_m3
      (float64 vol_to_extract_m3)                                 modification_; //150325_050303
      // This method removes the specified volume
      // or all of it if the volume is not satisfied
      // Returns the actual volume extracted
      // (incase there is not the requested amount).
   virtual float64 add_similar_biomatter
      (const Biomatter &added_biomatter)                          modification_; //100901
...
//_Biomatter_abstract_______________________________________________2010-08-20_/
