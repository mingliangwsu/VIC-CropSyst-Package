...
**********************************************************************/
{
  ...
  ***************************************************************/
  ...
  if(options.BINARY_OUTPUT) {  // BINARY
    // Initialize pointers
    size_t rec_num = N_OUTVAR_TYPES*options.Nlayer*options.SNOW_BAND;
    tmp_cptr = (char *)calloc(rec_num,sizeof(char));
    tmp_siptr = (short int *)calloc(rec_num,sizeof(short int));
    tmp_usiptr = (unsigned short int *)calloc(rec_num,sizeof(unsigned short int));
    tmp_iptr = (int *)calloc(rec_num,sizeof(int));
    tmp_fptr = (float *)calloc(rec_num,sizeof(float));
    tmp_dptr = (double *)calloc(rec_num,sizeof(double));
    ...
    for (int file_idx = 0; file_idx < options.Noutfiles; file_idx++) {
#if !OUTPUT_FORCE
      // Write the date
      if (dt < 24) {
      ...
      for (int var_idx = 0; var_idx < out_data_files[file_idx].nvars; var_idx++) {
        ...
        ptr_idx = 0;
        out_data_struct &out_data_rec = out_data[out_data_files[file_idx].varid[var_idx]];
        if (out_data_rec.type == OUT_TYPE_CHAR) {
          for (int elem_idx = 0; elem_idx < out_data_rec.nelem; elem_idx++) {
            tmp_cptr[ptr_idx++] = (char)out_data_rec.aggdata[elem_idx];
          }
          fwrite(tmp_cptr, sizeof(char), ptr_idx, out_data_files[file_idx].fh);
        }
        else if (out_data_rec.type == OUT_TYPE_SINT) {
          for (int elem_idx = 0; elem_idx < out_data_rec.nelem; elem_idx++) {
            tmp_siptr[ptr_idx++] = (short int)out_data_rec.aggdata[elem_idx];
          }
          fwrite(tmp_siptr, sizeof(short int), ptr_idx, out_data_files[file_idx].fh);
        }
        else if (out_data_rec.type == OUT_TYPE_USINT) {
          for (int elem_idx = 0; elem_idx < out_data_rec.nelem; elem_idx++) {
            tmp_usiptr[ptr_idx++] = (unsigned short int)out_data_rec.aggdata[elem_idx];
          }
          fwrite(tmp_usiptr, sizeof(unsigned short int), ptr_idx, out_data_files[file_idx].fh);
        }
        else if (out_data_rec.type == OUT_TYPE_INT) {
          for (int elem_idx = 0; elem_idx < out_data_rec.nelem; elem_idx++) {
            tmp_iptr[ptr_idx++] = (int)out_data_rec.aggdata[elem_idx];
          }
          fwrite(tmp_iptr, sizeof(int), ptr_idx, out_data_files[file_idx].fh);
        }
        else if (out_data_rec.type == OUT_TYPE_FLOAT) {
          for (int elem_idx = 0; elem_idx < out_data_rec.nelem; elem_idx++) {
            tmp_fptr[ptr_idx++] = (float)out_data_rec.aggdata[elem_idx];
          }
          fwrite(tmp_fptr, sizeof(float), ptr_idx, out_data_files[file_idx].fh);
        }
        else if (out_data_rec.type == OUT_TYPE_DOUBLE) {
          for (int elem_idx = 0; elem_idx < out_data_rec.nelem; elem_idx++) {
            tmp_dptr[ptr_idx++] = (double)out_data_rec.aggdata[elem_idx];
          }
    ...
    for (int file_idx = 0; file_idx < options.Noutfiles; file_idx++) {
      ...
      for (int var_idx = 0; var_idx < out_data_files[file_idx].nvars; var_idx++) {
        ...
        for (int elem_idx = 0; elem_idx < out_data_rec.nelem; elem_idx++) {
          ...
          fprintf(out_data_files[file_idx].fh, out_data_rec.format, out_data_rec.aggdata[elem_idx]);
        }
      }
      fprintf(out_data_files[file_idx].fh, "\n");
    }
