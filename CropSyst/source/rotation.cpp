...
: CORN::Item                                                                  ()
//, weather                                                             (weather_)
, weather_provider                                                           (0) //181224
#endif
, parameters                                                                  ()
, cyclable                                                               (false)
, template_rotation_file_name                               (rotation_filename_)
, clear_window_date                                                           () //181224
, sowing_precipitation_window_meter                              (STAT_ALL_bits) //181224
, avg_air_temp_for_window                                        (STAT_ALL_bits) //181224
, sowing_operation                                                           (0) //181224
, next_sowing_event                                                          (0) //181224
, cycle                                                                      (0) //180529
{
...
//_Rotation:constructor_____________________________________________2016-07-07_/
      ...
      {  for (nat8 sow_index = 0; sow_index < sowing_count ;sow_index++)
         {  CropSyst::Sowing_event *template_sow_event = sow_events[sow_index];
            ...
            {  CropSyst::Sowing_event *gen_event = new CropSyst::Sowing_event(); //130611
               ...
               gen_event->begin_sync.sync_mode_clad.set(ACTUAL_DATE_MODE);    //130611
...
//_render_planting_events___________________________________________2016-07-07_/
...
,nat8 cycle_)                                                               
{
   ...
   cycle = cycle_;
   start();                                                                 	 //180529
   weather_provider = const_cast <Weather_provider *>
      (&weather.ref_weather_provider());
   for (CORN::Date_clad_32 today(start_date)
...
//_render_planting_events_adjusted_for_weather_conditions___________2016-07-07_/
...
{  render_planting_events
   ...
   sowing_window_days_midpoint = sowing_window_days / 2;                         //170316brc
   ...
   {  plant_by_date_actual.set_date32(sowing_operation->plant_by_date);          //170602
      plant_by_date_actual.inc_years(start_date.get_year());                     //170602
   }
...
bool Rotation::process_day()                                       modification_
{
   if (simdate == clear_window_date)
      ...
      Sowing_date_mode planting_date_mode                                        //170331
         = sowing_operation->sowing_date_mode_clad.get();
      if (plant_by_date_actual.get_date32() && (simdate>=plant_by_date_actual))  //170602
      {  // event may specify date to plant by (0 disables)
         next_sowing_event->date.set(simdate);                                   //170602
      ...
      {  case MIN_TEMP_REQ    :
         {  float64 air_temp_avg = 0;
           ...
           if (next_sowing_event)                                                //181219
           {if ((simdate >= next_sowing_event->date)
                && (window_size == sowing_window_days)) // 15
            {  /* NYI && appreciable_rain_in_X_days*/
               if (warm_enough)
               {  CORN::Date_clad_32 computed_sowing_date(simdate);
                  computed_sowing_date.dec_days(sowing_window_days_midpoint);    //170316brc
                  #ifdef PLANTING_SCHEDULER
                  next_sowing_event->delay= next_sowing_event->date.days_between(computed_sowing_date);
                  #endif
                  next_sowing_event->date.set(computed_sowing_date);
                  //next_sowing_event->sync_mode_clad.set(ACTUAL_DATE_MODE);
                  next_sowing_event->begin_sync.set(computed_sowing_date);
                  next_sowing_event->begin_sync.sync_mode_clad.set(ACTUAL_DATE_MODE);
   ...
   //std::clog << "planting adjusted:" << next_sowing_event->date.as_string() << std::endl;
                  ...
                  if (next_sowing_event)
                  {
                     ...
                     if (sowing_operation->plant_by_date)                        //170602
                     {  plant_by_date_actual.set_date32(sowing_operation->plant_by_date); //170602
                        plant_by_date_actual.inc_years(simdate.get_year());      //170602
         ...
         {  // The other sowing date mode events a simply added to the planting events
            ...
            {
               std::cerr << "next_sowing_event is null" << std::endl;
            }
            */
         }
      }
...
//_resolve_crop_and_management______________________________________2017-09-29_/
Rotations::Rotations()
: CORN::Unidirectional_list(true) // unique
{}
//_Rotations:constructor____________________________________________2016-07-07_/
Rotation *Rotations::provide
...
//_Rotations::provide_______________________________________________2016-07-07_/
