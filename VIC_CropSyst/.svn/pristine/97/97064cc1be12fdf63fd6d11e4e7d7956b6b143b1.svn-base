...
static char vcid[] = "$Id: write_header.c,v 1.1.4.2 2007/10/11 22:40:08 vicadmin Exp $";
...
**********************************************************************/
{
  ...
  if(options.BINARY_OUTPUT) {  // BINARY
    ...
    for (file_idx = 0; file_idx < options.Noutfiles; file_idx++) {
      ...
      // nrecs
      fwrite(&(global_param.nrecs), sizeof(int), 1, out_data_files[file_idx].fh);

      // dt
      fwrite(&(global_param.out_dt), sizeof(int), 1, out_data_files[file_idx].fh);
