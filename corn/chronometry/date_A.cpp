...
(const Date_const &that_date,bool inclusive)                               const
{
   ...
   const Date_const &earlier_date
      ...
      ? that_date
      //200715LML: dynamic_cast<const Date &>(*this);                                       //120109
      : dynamic_cast<const Date_const &>(*this);
   const Date_const &later_date                                                  //120109
      = (this->get_date32() > that_date.get_date32())
      //200715LML ? dynamic_cast<const Date_interface &>(*this)
      ? dynamic_cast<const Date_const &>(*this)
      :  that_date;
...
// it should be easier to compose the text_str with this
   switch (ordering_BS)
   {
     ...
     case D_D  : target.append(doy_str);   break;                                //200402
   }
   if (DOY_indicator)
   {
      target += "(";
