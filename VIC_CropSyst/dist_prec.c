  ...
  **************************************************/
  ...
  if(options.DIST_PRCP) {
    ...
    ErrorFlag = full_energy(NEWCELL, cellnum, rec, atmos, prcp, &dmy[rec]/*150609LML dmy*/,global_param,
        lake_con, soil_con, veg_con, filep
        #ifdef VIC_CROPSYST_VERSION
        #if VIC_CROPSYST_VERSION==2
        ,crops
        #endif
  ...
  //************************************
  //  2026: the crop-state restore hook that used to live here has been
  //  MOVED to full_energy.c, between VIC_land_unit_start_day() and
  //  VIC_land_unit_process_day() for each veg tile's crop -- see that
  //  file's own comment at the call site for the full rationale
  //  (STARTDAY, for a warm-started run, is one calendar day after the
  //  saved state's own date, so that day's own, already-normal
  //  process_day() call needs the restore to have already happened
  //  immediately before it runs, not after this cell's entire
  //  full_energy() call -- including that same process_day() call --
  //  has already completed, as it did when this hook lived here).
  //************************************
