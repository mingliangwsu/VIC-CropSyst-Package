...
:Unidirectional_list()
{}
//_Lenses_manager:constructor__________________________________________________/
...
, bottom (bottom_)
{}
//_Lens:constructor____________________________________________________________/
float64 Soil_freezing_profile_jumakis::Lenses_manager::Lens::
get_fract_frozen(float64 from_depth , float64 to_depth ) const
{  float64 result = 0.0;
   if (!((top > to_depth) || (bottom < from_depth)
       || ((to_depth - from_depth) == 0.0)))  //981018
...
//_get_fract_frozen____________________________________________________________/
...
//_add_lens____________________________________________________________________/
...
//_get_highest_lens____________________________________________________________/
Soil_freezing_profile_jumakis::Lenses_manager::Lens *
...
//_get_next_highest_lens_______________________________________________________/
Soil_freezing_profile_jumakis::Lenses_manager::Lens *
...
//_get_lowest_lens_____________________________________________________________/
float64 Soil_freezing_profile_jumakis::Lenses_manager
::get_highest_lens_top()                                                   const
{  Lens *highest_lens = get_highest_lens();
   return (highest_lens )  ? highest_lens->top  : 0.0;
}
//_get_highest_lens_top________________________________________________________/
float64 Soil_freezing_profile_jumakis::Lenses_manager
::get_lowest_lens_bottom()                                                 const
{  Lens *lowest_lens = get_lowest_lens();
   return (lowest_lens) ? lowest_lens->bottom : 0.0;
}
//_get_lowest_lens_bottom______________________________________________________/
Soil_freezing_profile_jumakis::Lenses_manager::Lens *
...
//_get_next_lowest_lens________________________________________________________/
Soil_freezing_profile_jumakis::Soil_freezing_profile_jumakis                     //990308
(unmodifiable_ Soil::Layers_interface               &_soil_layers                //051201
,unmodifiable_ Soil::Structure                      &_soil_structure             //070725
,unmodifiable_ Soil::Hydraulic_properties_interface &_soil_hydraulic_properties  //051201
,  modifiable_ Soil::Hydrology_interface            &_soil_hydrology             //051201
,unmodifiable_ Soil::Temperature_interface          &_soil_temp_profile
,const         Air_temperature_average              &air_temperature_avg_        //151022
)          //080123
:Soil::Freezing_profile()                                                        //070825
...
{}
//_Soil_freezing_profile_jumakis:constructor________________________1999-03-08_/
float64 Soil_freezing_profile_jumakis::get_fract_frozen(nat8 layer) const
{ return frozen_lenses.get_fract_frozen
   (  soil_layers.get_depth_m(layer)
    - soil_layers.get_thickness_m(layer),soil_layers.get_depth_m(layer)); }
//_get_fract_frozen_________________________________________________1998-10-14_/
#ifdef OBSOLETE
...
{  if (direction == c)
      return 1.0 - depolarisation_factor(b,bulk_density,water_content)
                 - depolarisation_factor(a,bulk_density,water_content);
   else
...
//_depolarisation_factor_______________________________________________________/
#define continuous_medium  WATER
float64 directional_contribution
(directions direction,soil_components component
,float64 bulk_density,double_float water_content)
...
//_directional_contribution____________________________________________________/
...
//_avg_temp_grad_ratio_________________________________________________________/
...
//_thermal_conductivity________________________________________________________/
#endif
...
//_Soil_freezing_profile_jumakis____________________________________1998-06-12_/
...
//_get_frozen_thickness_m______________________________________________________/
...
{  float64 fract_frozen = get_fract_frozen(layer);  //  portion of the soil that contains some ice and water
   ...
   ice_content_volumetric =
            CORN::must_be_0_or_greater<float64>((all_water_content- perm_wilt_point_sl))
                      * fract_frozen;
...
//_calc_liquid_and_ice_water_content_volumentric____________________1999-03-11_/
...
void Soil_freezing_profile_jumakis::perform_freezing()
{
   ...
   for (int8 layer = 1; layer <= soil_layers.count(); layer++)
   ...
   {  frozen_layer ++;
      ...
      (frozen_layer != soil_layers.count()));
   float64 total_frost_depth_cm = 0.0;
   if (frozen_layer == soil_layers.count())
...
//_perform_freezing_________________________________________________2007-08-08_/
...
,soil_layer_array64(required_thawing_index))
{
   ...
   {  thawing_index_left -= layer_require_thawing_index;
      ...
      ? (layer == bottom_layer)
      : (layer == top_layer);
      layer_require_thawing_index = CORN::must_be_0_or_greater<float64>
             (required_thawing_index[layer] - prev_layer_required_thawing_index);
      prev_layer_required_thawing_index = required_thawing_index[layer] ;        //000711
      if (layer < soil_layers.count())                                           //070827
         layer_freeze_or_thaw_occured[layer] = thawed_in_layer > 0.0 ;           //070827
    }
    if (layer_require_thawing_index != 0)
    {
      float64 thawed_in_layer =
         thawing_index_left * soil_layers.get_thickness_m((nat8)layer)
                              / (layer_require_thawing_index);
      total_thawed += thawed_in_layer;
      layer_freeze_or_thaw_occured[layer] = thawed_in_layer > 0.0 ;              //070827
    }
...
//_thaw_____________________________________________________________1998-10-14_/
...
{  float64 thawing_index = - freezing_index;                                     //981002
   ...
   int8 last_frozen_layer = soil_layers.count();                                 //981015
   for (int8 check_frozen_layer = 1
       ; check_frozen_layer <= soil_layers.count()
...
//_perform_thawing__________________________________________________1998-10-15_/
...
//_freezing_and_thawing_____________________________________________1998-10-15_/
bool Soil_freezing_profile_jumakis::is_frozen(nat8 layer)                  const
{  bool frozen = get_fract_frozen(layer) > 0.5;
   return frozen;
}
//_is_frozen________________________________________________________1998-10-14_/
