...
, encountered(false)
{}
//_VV_entry_item:construtcor________________________________________2009-04-14_/
...
{}
//_VV_base_entry:constructor___________________________________________________/
...
//_append_annotation_to_____________________________________________2015-07-31_/
...
//_VV_base_entry::label_string______________________________________2017-04-23_/
...
//_VV_base_entry::write________________________________________________________/
...
{ return variable == key; }
//_get_key__________________________________________________________2003-01-17_/
...
//_VV_nat8_entry____________________________________________________2014-06-15_/
...
, value(value_)
{}
//_VV_byte_entry:constructor___________________________________________________/
...
{}
//_VV_bool_entry:constructor________________________________________1997-06-22_/
...
, value(value_)
{}
//_VV_int16_entry:constructor_______________________________________1997-06-22_/
...
, radix(_radix)
{}
//_VV_nat16_entry___________________________________________________1997-06-22_/
...
//_VV_nat16_entry::set_str__________________________________________2001-01-03_/
...
, value(value_)
{}
//_VV_int32_entry:constructor__________________________________________________/
...
{  value = (float32)atof(value_str);
   set_validation_status
      (!is_number_cstr(value_str,'.')                                            //190716
         // Warning presuming . is decimal point should localize
       ?missing_status:good_status);                                             //160215_050517
   return true;                                                                  //090414
}
...
{  value = cstr_to_float64(value_str);
   set_validation_status(!is_number_cstr(value_str,'.')                          //190716
      // Warning presuming . is decimal point should localize
      ?missing_status:good_status);                                              //160214-050517
   return true;                                                                  //090414
}
...
{
   if (is_number_cstr(value_str,0))                                              //190716
      value.set_date32(atol(value_str));                                         //190716
   else                                                                          //190716
   //190716  if (formatted)
      value.set_c_str(value_str);
   //190716 else           value.set_date32(atol(value_str));                             //050412
   return true;
}
//_VV_date_entry::set_str___________________________________________2001-01-03_/
bool VV_date_entry::apply_offset_years(int16 year_offset)         modification_
{  bool applied = false;
...
//__VV_date_entry::apply_years_offset_______________________________2018-10-01_/
...
, value(value_)
, formatted(true)
{  date_format.set_cstr(format_);
   //NYI time_format
}
//_VV_datetime64_entry::constructor_________________________________2019-05-14_/
bool VV_datetime64_entry::apply_offset_years(int16 year_offset)         modification_
{  bool applied = false;
   if (year_offset)                                                              //120331
   {
      value
...
//_VV_date_entry::apply_years_offset________________________________2018-10-01_/
bool VV_datetime64_entry::set_str(const char *value_str)
{
   if (is_number_cstr(value_str,'.'))                                            //190716
      // warning presuming . is decimal point. Should localize
      value.set_datetime64(atof(value_str));                                     //190716
      // atof converts to double (might not be the case in old compilers!)    //050411
   else
   //190716 if (formatted)
      value.set_c_str(value_str);
   //190716    else           value.set_datetime64(atof(value_str));  // atof converts to double (might not be the case in old compilers!)   //050411
   set_validation_status(!is_number_cstr(value_str,'.')                          //190716
      ?missing_status:good_status);                                              //160215_050517
   return true;                                                                  //090414
}
...
(const char* key_
,Enum_abstract &enum_clad_                                                       //190109
,const char* annotation1_, const char* annotation2_)
: VV_base_entry                             (key_,80,annotation1_, annotation2_)
, enum_clad                                                         (enum_clad_)
{   // Now determine the actual max_strlen from the label table
    max_strlen = enum_clad.get_max_label_length();                               //190109_050929
}
...
{   enum_clad.set_label(value_label_);                                           //190109_020322
}
...
{  enum_clad.append_label(buffer);                                               //190109
   return buffer;
}
...
{}
//_VV_units_entry______________________________________________________________/
...
//_VV_Section:label_string_____________________________________________________/
bool VV_Section::write(std::ostream &f)                            modification_
{  nat32 written = 0;
   if (!entries.is_empty())                                                      //040109
   { f << '[' << label << ']' << std::endl;                                      //180821
     written = entries.write(f);
   }
   return written;
}
//_VV_Section::write________________________________________________2015-07-28_/
VV_base_entry *VV_Section::set_entry
(const char *key                                                                 //030117
,const char *value                                                               //030117
,bool unexpected_only)
{  if (!key || key[0] == 0) return 0;                                            //200301
...
//_VV_directory_entry_entry::set________________________________________________
void VV_directory_entry_entry::set(const std::string &value_)
{  value.set_cstr(value_.c_str());
}
//_VV_directory_entry_entry::set____________________________________1999-08-24_/
bool VV_directory_entry_entry::set_str(const char *value_str)
{ value.set_cstr(value_str); return true;}
//_VV_directory_entry_entry::set_str________________________________2001-01-03_/
...
//_VV_parameter_entry::is_key_string___________________________________________/
bool VV_parameter_entry::set_str(const char *value_cstr)           modification_
{ return parameter.set_str(value_cstr); }
//______________________________________________________________________________
const std::string &VV_parameter_entry::append_variable(std::string &variable_return) const
{  variable_return.append(parameter.properties.name_long);                       //181115
   return variable_return;                                                       //181115
}
...
//_VV_parameter_entry::write___________________________________________________/
...
{  const Parameter_number & parameter_number = dynamic_cast
   ...
   const char* param_prop_ontology = parameter.properties.ontology;
   if (param_prop_ontology && strlen(param_prop_ontology))
      wizard_stream << "ontology=" << param_prop_ontology << std::endl;
/*190105
...
{ return parameter.format.delineation.width; }                                   //200224
