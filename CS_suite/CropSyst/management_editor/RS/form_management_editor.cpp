...
(CropSyst::Management_parameters *_management,Tparameter_file_form      *_parameter_file_form)
{
   ...
   radiogroup_land_treatment  ->bind_to(&(management->land_treatment_clad)/*NYI,parameter_file_form->compose_manual_subdirectory_filename(HELP_P_mgmt_conserv_land_treatment).c_str()*/);
...
bool TManagement_editor_form::add_event_to_listview_operations(Common_event *added_event)
{
   ...
   if (op)
   {
      ...
      CropSyst_Op_descriptive_clad op_type_clad(op_type);
      switch (op_type)
      ...
      std::string op_desc;
      event_item->Caption = UnicodeString(op_type_clad.append_description(op_desc));
      //190114 event_item->Caption = UnicodeString(op_type_clad.get_description());
   }
...
{
refill_operations(management->residue_operations); // <- debug only
   return
...
//_refill_operations_listview_______________________________________2017-07-04_/
...
//_refill_operations________________________________________________2017-07-04_/
