...
)                                                                   modification_
{
   ...
   // separate fix above) -- that is a SEPARATE, per-period tracker, not
   // derived from this global accumulator. This fix instead addresses
   // thermal_time_matchs() (which gates
...
{  bool considered = true;                                                       //190709
   ...
   if (   quiescence_start_requirement
      ...
      // for end season before we are even growing.                              //080401
       && ((parameters->is_fruit_tree() && !phenology.is_in_accrescence())
