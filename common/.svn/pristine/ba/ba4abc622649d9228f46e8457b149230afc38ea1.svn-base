...
//_Weather_base:constructor_________________________________________1999-03-01_/
Weather_base::~Weather_base()
...
//_Weather_base:destructor__________________________________________2000-07-10_/
...
#endif
//_get_location_parameters__________________________________________2005-07-13_/
...
//_load_wind_speed_estimate_if_necessary________________2015-01-26__1997-03-03_/
/*200326 obsolete
...
{  float64 result = 0.0;
   ...
   switch (location?location->wind_speed_classification_clad.get():WIND_SPEED_MEDIUM )
...
//_forecast_wind_speed_m_d__________________________________________1997-03-03_/
*/
...
//_get_description__________________________________________________2004-11-09_/
void Weather_base::load()
{
   ...
   if (treat_valid_data_as_real)                                                 //081109
   {
      ...
      if (provider.precipitation_actual   .is_valid()) provider.precipitation_actual .assume_code(measured_quality);
      if (provider.solar_radiation        .is_valid()) provider.solar_radiation      .assume_code(measured_quality);  //081109
      if (provider.wind_speed             .is_valid()) provider.wind_speed           .assume_code(measured_quality);  //081109
      if (curr_max_temperature            .is_valid()) curr_max_temperature          .assume_code(measured_quality);  //081109
      if (curr_min_temperature            .is_valid()) curr_min_temperature          .assume_code(measured_quality);  //081109
      if (curr_max_dew_point_temperature  .is_valid()) curr_max_dew_point_temperature.assume_code(measured_quality);  //081109
      if (curr_min_dew_point_temperature  .is_valid()) curr_min_dew_point_temperature.assume_code(measured_quality);  //081109
      if (curr_avg_dew_point_temperature  .is_valid()) curr_avg_dew_point_temperature.assume_code(measured_quality);  //081109
      if (curr_max_relative_humidity      .is_valid()) curr_max_relative_humidity    .assume_code(measured_quality);  //081109
      if (curr_min_relative_humidity      .is_valid()) curr_min_relative_humidity    .assume_code(measured_quality);  //081109
      if (provider.ET_reference           .is_valid()) provider.ET_reference         .assume_code(measured_quality);  //180806
   }
}
...
//_commit___________________________________________________________2005-06-16_/
...
//_fix_up_relative_humidity____________________________________________________/
...
//_any_valid_data_today________________________________________________________/
/*200326
float64 Weather_base::get_curr_max_air_temperature()                       const
{return curr_max_temperature;}                                                   //140916LML
//_get_curr_max_air_temperature________________________________________________/
*/
float64 Weather_base::get_air_temperature_min_tomorrow()                   const
{  return Tmin_tomorrow;
}
//_get_air_temperature_min_tomorrow_________________________________2015-12-01_/
float64 Weather_base::get_air_temperature_max_yesterday()                  const
{  return Tmax_yesterday;
}
//_get_air_temperature_max_yesterday________________________________2015-12-01_/
#ifdef WEATHER_GENERATION
...
//_get_earliest_available_date______________________________________2014-02-09_/
...
//_setup_property_reference_________________________________________2015-11-19_/
*/
