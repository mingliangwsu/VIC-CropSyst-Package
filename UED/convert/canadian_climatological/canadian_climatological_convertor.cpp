...
int Canadian_climatological_format_convertor::convert()
{
   ...
   for (int i = 0; i <= 172; i++)
   {
      ...
      {  // Create  nonstandard code based on the element number
         Variable_code_clad element_based_code(0,element_conversions[i].element_number,true);
...
//_class Canadian_element_record_______________________________________________/
...
public:
   ...
   ,Variable_code_clad    i_variable_code
...
/*_______*/       {
/*_______*/          UED::Variable_code_clad element_var_code(element_conversions[i].var_code);
...
/*_______*/ }
//______________________________________________________________________________
nat32 Canadian_climatological_format_convertor::perform_export()
{
   nat32 records_exported = 0;                                                   //060616
   cout << "Openning and indexing database..." << '\r';

   UED_indexed_database ued_file(parameters.ued_filename.c_str(),true);
   CORN::Quality_code datum_attribute;


   // First assume the UED_filename file matches the station ID:
   ...
   CORN::File_name wind_filename(parameters.ued_filename.c_str());
   wind_filename.set_name(filename_only);
   wind_filename.set_ext("windspeed");
   ofstream wind_output(wind_filename.c_str());
   // This is a special output file for  "Rafael Munoz-Marquez" <ramunozm@sympatico.ca>
   wind_output << "Date" << '\t' << "Windspeed"<< endl;
   wind_output << "YYYYDDD" << '\t' << "km/day"<< endl;

   // station ID taken from UED file location record, if it exists, but currently import wizard prompts user for it

   // Get the location record for the UED

   const UED_geolocation_record  *location_rec= ued_file.ref_geolocation_record(); //190515_060616
   ...
   for  (UED_data_record *data_rec = ued_file.goto_next_data_record();data_rec ;data_rec = ued_file.goto_next_data_record())
   {
      ...
      Element element = 0;
      // need to convert data_rec variable to code element code
      UED_variable_code variable_code = data_rec->get_variable_code();
      int element_index = 0;
      for (int i = 1;element_conversions[i].element_number && !element;i++)
         if (element_conversions[i].var_code == variable_code)
         {
            element = element_conversions[i].element_number;
            element_index = i;
         }
      cout << for_time.get_year();
      cout << " " << element_conversions[element_index].description << endl;
      for (int TS_count = 1; TS_count <= num_values; TS_count++)
      {
         if (time_step_units_code == UT_day)
         {  // record is daily resolution
            // add values to Canadian_daily_by_month_data_record list:
             // get Canadian_daily_by_month_data_record from list
            // that can store the data value for the time step
            // (create if necessary)
            float32 value = data_rec->get_value
            (datum_attribute
            ,for_time.get_datetime64());
            if (variable_code == UED_STD_VC_wind_speed.get())
            {  // we need to  convert to from daily values to hourly
               // because  canadian climatological format does not have daily windspeed.
               // Canadian climatological does not have daily windspeed
               // These need to be converted to hourly for the .HLY15 file.
              float32 value_km_day = value;
              wind_output << for_time.get_date32() << '\t' << value_km_day << endl; //031221_
